...
function native(c,provider,raw={}){
 ...
 return {names:[{...(raw.names?.[0]||{}),givenName:c.firstName,familyName:c.lastName,honorificPrefix:c.title}],organizations:[{...(raw.organizations?.[0]||{}),name:c.institution},...(raw.organizations||[]).slice(1)],addresses:[{...(raw.addresses?.[0]||{}),streetAddress:street,city:c.city,postalCode:c.postal,country:c.country},...(raw.addresses||[]).slice(1)],emailAddresses:emails.map(w=>({value:w.value,type:w.label})),phoneNumbers:ways.filter(w=>w.type!=='email').map(w=>({value:w.value,type:w.type==='mobile'?'mobile':w.type==='fax'?'workFax':'main',formattedType:w.label}))};
}
...
function unpack(extra,current){
 ...
 if(!same(current.contactWays,baseline.contactWays)){
  ...
  const seen=new Set();out.contactWays=out.contactWays.filter(w=>{const key=w.type+'|'+w.value;if(seen.has(key))return false;seen.add(key);return true});for(const type of Object.keys(T.TYPES)){let found=false;for(const w of out.contactWays.filter(w=>w.type===type)){w.preferred=!found&&w.value===out[type];if(w.preferred)found=true}}
 }
 return out;
}
...
async function save(conn,book,binding,data,remote){
 ...
 if(['nextcloud','icloud'].includes(conn.provider)){
  ...
  const line=codec.fold('X-ADDRESSBOOK-SYNC:'+JSON.stringify(extra).replace(/\\/g,'\\\\').replace(/\n/g,'\\n').replace(/;/g,'\\;').replace(/,/g,'\\,'));const retained=(remote?.rawVcard||'').replace(/\r?\n[ \t]/g,'').split(/\r?\n/).filter(l=>{const key=l.split(':')[0].split(';')[0].split('.').at(-1).toUpperCase();return key&&!['BEGIN','END','VERSION','UID','FN','N','ORG','ADR','LABEL','EMAIL','TEL','AGENT','X-ABLABEL','X-BETREUUNGSBUERO-DATA','X-ADDRESSBOOK-SYNC'].includes(key)});const end=vcard.lastIndexOf('END:VCARD');const body=vcard.slice(0,end)+retained.map(l=>codec.fold(l)).join('\r\n')+'\r\n'+line+'\r\nEND:VCARD\r\n';
