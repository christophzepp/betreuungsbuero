...
function restore(input,session){const events=[],warnings=[];let target;
 ...
 const {rows,links}=read(item);for(const r of rows){if(r.scope==='case'&&!db.prepare('SELECT id FROM live_cases WHERE id=?').get(r.case_id))A.fail(409,'Ein zugehöriger Fall existiert nicht mehr.');if(A.get(r.scope,r.case_id||'',r.id))A.fail(409,'Die ursprüngliche Kontakt-ID ist bereits belegt. Es wurden keine Daten überschrieben.')}
