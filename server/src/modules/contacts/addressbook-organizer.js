...
function quality(b,s){
 ...
 const records=[...db.prepare('SELECT id,data_json FROM office_contacts').all().map(r=>({...r,scope:'office',caseId:''})),...db.prepare('SELECT c.id,c.case_id,c.data_json FROM live_case_contacts c LEFT JOIN addressbook_links l ON l.case_contact_id=c.id WHERE l.case_contact_id IS NULL').all().filter(r=>canRead('case',r.case_id,s)).map(r=>({...r,scope:'case',caseId:r.case_id}))];
