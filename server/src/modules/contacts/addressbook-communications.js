...
function page({scope,caseId,id,session,cursor,ids,refs,centralId,contact}){
 ...
 const linked=(l,cid)=>l&&(l.scope==='office'?l.contactId===(centralId||(scope==='office'?id:'')):(l.scope||'case')==='case'&&ids.has(l.contactId)&&(!l.caseId||l.caseId===cid));
 const allContacts=db.prepare('SELECT id,case_id,data_json FROM case_contacts').all().map(r=>({...parse(r.data_json),id:r.id,caseId:r.case_id}));
 const office=db.prepare('SELECT id,data_json FROM office_contacts').all().map(r=>({...parse(r.data_json),id:r.id,caseId:''}));
 const candidates=[...allContacts,...office],target=emails(contact);
 const history=db.prepare('SELECT contact_id,case_id,changes_json FROM addressbook_history WHERE contact_id IN (SELECT value FROM json_each(?))').all(JSON.stringify([...ids]));for(const h of history){if(h.case_id&&!caseMap.has(h.case_id))continue;const ch=parse(h.changes_json),oldEmails=[ch.email?.before,ch.email?.after,...(ch.people?.before||[]).map(p=>p.email),...(ch.people?.after||[]).map(p=>p.email)].map(email).filter(Boolean);for(const e of oldEmails)target.add(e);if(oldEmails.length)candidates.push({id:h.contact_id,caseId:h.case_id,oldEmails})}
 // Nur eindeutige E-Mail-Zuordnungen; ein gemeinsam genutztes Postfach ist kein Identitätsnachweis.
 const globalEmails=new Map(),caseEmails=new Map();for(const c of candidates)for(const addr of emails(c)){const g=globalEmails.get(addr)||new Set();g.add(c.id);globalEmails.set(addr,g);if(c.caseId){const key=c.caseId+'|'+addr,local=caseEmails.get(key)||new Set();local.add(c.id);caseEmails.set(key,local)}}
 function matches(addresses,cid){for(const addr of addresses){if(!target.has(addr))continue;const hits=cid?caseEmails.get(cid+'|'+addr):globalEmails.get(addr);if(hits?.size&&[...hits].every(id=>ids.has(id)))return true}return false}
 const norm=v=>String(v||'').toLocaleLowerCase('de-DE').replace(/[,;–—]/g,' ').replace(/\s+/g,' ').trim();
 function matchesRecipient(value,cid){const v=norm(value);if(!v)return false;const hits=allContacts.filter(c=>c.caseId===cid).filter(c=>{const person=[c.title,c.firstName,c.lastName].filter(Boolean).join(' '),base=[c.institution,person].filter(Boolean),address=[[c.street,[c.house,c.houseLetter].filter(Boolean).join('')].filter(Boolean).join(' ')||(c.postbox?'Postfach '+c.postbox:''),[c.postal,c.city].filter(Boolean).join(' ')];return [base.join(' '),[...base,...address].filter(Boolean).join(' ')].some(s=>norm(s)===v)});return hits.length>0&&hits.every(c=>ids.has(c.id))}
 ...
 for(const t of db.prepare("SELECT * FROM todos WHERE item_type='followup'").all()){
  ...
  if(!link&&d?.caseId===t.case_id)link=d.contactLink;
...
async function sync(input,session,engines){
 ...
 job.pending=(async()=>{
  ...
  while(next.account<next.accounts.length){
   ...
   try{
    if(!next.folders){next.folders=(await (engine.listFoldersComplete||engine.listFolders)(acc)).filter(f=>f.path&&!f.noSelect);next.folder=0;next.offset=0;next.seenUids=[]}
    if(next.folder>=next.folders.length){for(const table of ['mail_cache','addressbook_mail_index'])db.prepare(`DELETE FROM ${table} WHERE account_id=? AND folder NOT IN (SELECT value FROM json_each(?))`).run(acc.id,JSON.stringify(next.folders.map(f=>f.path)));next.account++;next.folders=null;continue}
    ...
    if(!messages.length||next.offset>=result.total){db.prepare('DELETE FROM addressbook_mail_index WHERE account_id=? AND folder=? AND uid NOT IN (SELECT value FROM json_each(?))').run(acc.id,folder,JSON.stringify(next.seenUids));db.prepare('DELETE FROM mail_cache WHERE account_id=? AND folder=? AND uid NOT IN (SELECT uid FROM addressbook_mail_index WHERE account_id=? AND folder=?)').run(acc.id,folder,acc.id,folder);next.folder++;next.offset=0;next.seenUids=[]}
