...
function find(input,session){
 ...
 const records=[...db.prepare('SELECT * FROM office_contacts').all().map(r=>({...r,scope:'office',case_id:''})),...db.prepare('SELECT c.*,f.label case_label FROM live_case_contacts c JOIN live_cases f ON f.id=c.case_id').all().filter(r=>darfSehen(session,r.case_id)).map(r=>({...r,scope:'case'}))];
