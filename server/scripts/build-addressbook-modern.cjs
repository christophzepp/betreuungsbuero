...
for(const [kind,close] of [['css','</style>'],['js','</script>']]){
 ...
 if(kind==='js')source=source.replace('/* ADDRESSBOOK-ASSIGNMENT-UI */',fs.readFileSync(path.resolve(__dirname,'../frontend/addressbook-assignment-ui.js'),'utf8'));
 if(kind==='css')source+='\n'+fs.readFileSync(path.resolve(__dirname,'../frontend/addressbook-extras.css'),'utf8');
 if(kind==='js')source=fs.readFileSync(path.resolve(__dirname,'../frontend/addressbook-assignment-data.js'),'utf8')+'\n'+fs.readFileSync(path.resolve(__dirname,'../frontend/addressbook-extras-data.js'),'utf8')+'\n'+fs.readFileSync(path.resolve(__dirname,'../frontend/addressbook-contact-tools.js'),'utf8')+'\n'+fs.readFileSync(path.resolve(__dirname,'../frontend/addressbook-vcard.js'),'utf8')+'\n'+source;
 const block=start+'\n'+source+'\n'+end;
