...
(async()=>{
 ...
 await check('Originalfilter, Auswahl, Import und Export bleiben vorhanden',async()=>{
  ...
  await page.locator('.am-menu summary').click();for(const t of ['Adressbuch als Excel herunterladen','vCard exportieren','vCard importieren','Duplikate & Kontakte zusammenführen'])assert.ok((await page.locator('.am-menu-body').innerText()).includes(t),t);await page.locator('.am-menu summary').click();
 });
 ...
 await check('vCard-Import wird dauerhaft gespeichert und Duplikate werden übersprungen',async()=>{
  await page.locator('.am-menu summary').click();const input=page.locator('.am-menu-body input[type=file]'),file={name:'kontakt.vcf',mimeType:'text/vcard',buffer:Buffer.from('BEGIN:VCARD\r\nVERSION:3.0\r\nFN:Vera Import\r\nN:Import;Vera;;;\r\nEMAIL:import@example.org\r\nEND:VCARD\r\n')};await input.setInputFiles(file);await page.waitForFunction(()=>state.caseData.contacts.some(c=>c.email==='import@example.org'&&c.id));const imported=db.prepare("SELECT id FROM case_contacts WHERE json_extract(data_json,'$.email')='import@example.org'").all();assert.equal(imported.length,1);await page.locator('.am-menu summary').click();await page.locator('.am-menu-body input[type=file]').setInputFiles(file);await page.waitForTimeout(400);assert.equal(db.prepare("SELECT count(*) n FROM case_contacts WHERE json_extract(data_json,'$.email')='import@example.org'").get().n,1);
 });
