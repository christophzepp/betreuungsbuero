...
(async()=>{
for(const [name,navFixture,expected] of [['neu',{},['case-overview','documentation','mail']],['lokales-altprofil',{local:custom},['mail','tasks','calendar']],['server-altprofil',{local:custom,server:legacy},['case-overview','documentation','mail']]]){
 ...
 const favorites=()=>nav.locator('.mobile-nav-favorites [data-mobile-action]').evaluateAll(es=>es.map(e=>e.dataset.mobileAction));
 ...
 try{
  ...
  check(name+': Start genau einmal fest außerhalb der Favoriten',await nav.locator(':scope > [data-mobile-action="start"]').count()===1&&await nav.locator('[data-mobile-action="start"]').count()===1);
  ...
  check(name+': Editor ohne alten Chat und mit festem Start',await sheet.locator('[data-editor-id="case-chat"]').count()===0&&await sheet.locator('[data-editor-id="start"] .mobile-pin-toggle').isDisabled());
  check(name+': Vorschau zeigt Start, Chats und Mehr',await sheet.locator('.mobile-nav-preview .is-static').allTextContents().then(text=>text.join(',')==='Start,Chats,Mehr'));
  ...
  check('Start belegt keinen der acht Favoritenplätze',await sheet.locator('[data-preview-id]').count()===8&&await sheet.locator('[data-editor-id="banking"] .mobile-pin-toggle').getAttribute('aria-pressed')==='false');
  await sheet.locator('[data-mobile-editor-save]').click();await sheet.locator('[data-mobile-sheet-close]').click();
  ...
  for(const width of [320,390,570])for(const dark of [false,true]){
   ...
   check('Feste Zugänge bleiben sichtbar bei '+width+' / '+(dark?'dunkel':'hell'),await nav.evaluate(e=>[...e.querySelectorAll(':scope > button')].every(b=>{const r=b.getBoundingClientRect();return r.left>=0&&r.right<=innerWidth&&r.width>=44&&r.height>=44})&&document.documentElement.scrollWidth<=innerWidth+1));
  ...
  await page.setViewportSize({width:390,height:844});await page.evaluate(()=>{document.documentElement.dataset.theme='light';window.__userChat={getState:()=>({available:true,unread:3,unreadConversations:2,open:false}),close:()=>{},open:()=>window.__qaTeamOpened=true};dispatchEvent(new Event('userChatStateChanged'));const c=ensureAIConfig();c.provider='openai';c.apiKey='fixture-no-network';aiProviderKeyConfigured=()=>true});
