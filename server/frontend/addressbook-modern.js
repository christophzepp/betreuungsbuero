...
function drawTab(c){
 ...
 if(M.tab==='addresses'){drawAddresses(body,c,data);return}else if(M.tab==='availability'){drawAvailability(body,c,data);return}else if(M.tab==='people'){
  ...
  for(const item of M.bundle.communications){const s=section(item.title);s.append(E('p','am-sub',[item.date,item.caseLabel,item.contactType,item.person].filter(Boolean).join(' · ')),E('p','am-note',item.text),B(({mail:'E-Mail öffnen',document:'Versandhistorie öffnen',followup:'Wiedervorlage öffnen'})[item.kind]||'In Falldokumentation öffnen',()=>openCommunication(item)));body.append(s)}
...
function contactForm(c,title,fields,toPatch){
 let version=M.bundle.version;smallForm(title,fields,async values=>{const patch=toPatch(values);let bundle;if(online())bundle=await request('contact',{...ref(c),version,patch},'PATCH');else bundle=await savePatch(c,patch);version=bundle.version;await cacheBundle(bundle,ref(c));M.bundle=bundle},async()=>{throw Error('Bitte offene Eingaben kopieren und verwerfen; anschließend den aktuellen Kontakt erneut bearbeiten. Andere Änderungen wurden nicht überschrieben.')});
}
...
async function openContact(id){const c=api().contacts().find(c=>c.id===id);if(c){await choose(c);return}await window.__abSwitchCase('__buero');const r=M.rows.find(c=>c.id===id);if(r)await choose(r);else message('Die Vertretung ist im zentralen Adressbuch nicht mehr verfügbar.',true)}
function editAssociation(c,a){let version=a.version;smallForm('Fallangaben: '+a.label,['role','fileNumber','processNumber','customerNumber'].map(k=>[k,LABELS[k],a[k]]),async values=>{const bundle=await request('contact',{scope:'case',caseId:a.caseId,id:a.contactId,version,patch:values},'PATCH');version=bundle.version;await cacheBundle(bundle,{scope:'case',caseId:a.caseId,id:a.contactId});M.bundle=await request('contact?'+new URLSearchParams(ref(c)))},async()=>{throw Error('Die Fallangaben wurden geändert. Bitte offene Eingaben kopieren und den aktuellen Stand erneut öffnen.')})}
...
async function openCommunication(item){if(item.kind==='mail'){window.__mxOpenMsg(item.accountId,item.folder,item.uid);return}if(item.kind==='document'){if(item.caseId!==activeCase())await window.__abSwitchCase(item.caseId);if(item.caseId!==activeCase())throw Error('Der zugehörige Fall konnte nicht geöffnet werden.');window.showExportHistory();return}if(item.kind==='followup'){window.openFollowupsWorkspace();return}return openDoku(item)}
async function syncMail(c,force=false){
 if(!online()||M.tab!=='communication')return;const key=identity(c);if(M.mailSync?.running||(!force&&M.mailSync?.key===key&&Date.now()-M.mailSync.at<60000))return;
 const run=M.mailSync={key,running:true,at:Date.now(),text:'Nachrichten werden abgeglichen …'};let cursor;try{do{const result=await request('communications/sync',{...ref(c),...(cursor?{cursor}:{})});cursor=result.cursor;run.text=result.done?(result.errors.length?'Teilweise aktualisiert · '+result.errors.join(' '):'Nachrichten aktuell · '+result.checked+' geprüft'):'Nachrichtenabgleich läuft · '+result.checked+' geprüft';const el=$('#amMailSync');if(el&&M.selected===key)el.textContent=run.text;if(result.done&&M.selected===key&&!M.small&&!M.editor){const result=await request('communications?'+new URLSearchParams(ref(c)));M.bundle.communications=result.items;M.bundle.communicationCursor=result.nextCursor;drawTab(c)}}while(cursor&&M.root?.isConnected&&M.selected===key&&M.tab==='communication');if(cursor)run.text='Abgleich unterbrochen · erneut aktualisieren';}catch(e){run.text='Nachrichten konnten nicht vollständig aktualisiert werden.';const el=$('#amMailSync');if(el)el.textContent=run.text;throw e}finally{run.running=false}}
