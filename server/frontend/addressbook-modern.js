...
async function choose(c){if(!await flush())return;M.editRequest++;M.editor=null;M.utility=false;M.selected=identity(c);M.tab='data';M.root.classList.add('am-show-detail');await X.used(c);window.phase5RenderAddressbookV154()}
...
function drawDetail(c){
 ...
 const tabs=E('nav','am-tabs');tabs.setAttribute('aria-label','Kontaktdetails');for(const [key,label,short] of [['data','Kontaktdaten','Kontakt'],['cases','Fälle & Standard','Fälle'],['people','Ansprechpartner','Personen'],['addresses','Anschriften','Adressen'],['ways','Kontaktwege','Kontaktwege'],['extras','Organisation & Extras','Extras'],['sync','Synchronisation','Sync'],['availability','Erreichbarkeit','Erreichbar'],['history','Änderungen','Änderungen'],['communication','Kommunikation','Verlauf']]){if(c.__viewMergeId&&key!=='data')continue;const b=B('',()=>{M.tab=key;drawTab(c);if(innerWidth<=680)requestAnimationFrame(()=>{const pane=$('.am-detail'),tabs=$('.am-tabs');if(pane&&tabs)pane.scrollTop+=tabs.getBoundingClientRect().top-pane.getBoundingClientRect().top})});b.dataset.tab=key;b.setAttribute('aria-label',label);b.title=label;b.append(contactIcon(key),E('span','am-tab-label',short));tabs.append(b)}const caption=E('p','am-tab-caption');caption.setAttribute('aria-hidden','true');pane.append(tabs,caption,E('div','am-tab-body'));
...
function drawTab(c){
 ...
 if(M.tab==='extras'){X.metadata(body,c,data);return}else if(M.tab==='sync'){drawSync(body,c);return}else if(M.tab==='ways'){drawWays(body,c,data);return}else if(M.tab==='addresses'){drawAddresses(body,c,data);return}else if(M.tab==='availability'){drawAvailability(body,c,data);return}else if(M.tab==='people'){
...
async function edit(c,initial={}){M.utility=false;
 ...
 for(const [title,fields] of GROUPS){const group=section(title),grid=E('div','am-form-grid');for(const k of fields){if(k==='customerNumber'&&r.scope==='office')continue;const l=E('label','',LABELS[k]);let input;
  ...
  else input=E(k==='note'?'textarea':'input');input.name=k;input.id='amEdit_'+k;input.value=draft[k]||'';if(k==='email')input.type='email';if(k==='note'){l.className='am-note-field';l.textContent='';input.className='am-note-input';input.rows=2;input.setAttribute('aria-label','Notizen');input.title='Wächst beim Schreiben. Am unteren rechten Rand mit der Maus vergrößerbar.'}
...
function changed(k,v){const e=M.editor;if(!e)return;if(k==='_category')window.__abModernSetupCombos?.(v);e.draft[k]=v;e.pending[k]=v;e.error=false;e.allowDuplicates='';
 ...
 clearTimeout(e.timer);message(online()?'Änderungen noch nicht gespeichert …':'Lokale Änderungen noch nicht übernommen …');if(e.new){message('Neuer Kontakt · Angaben prüfen und Kontakt anlegen');e.timer=setTimeout(()=>checkDuplicates(e),300)}else if(online())e.timer=setTimeout(()=>savePending(),650);
}
...
function drawWays(body,c,d){
 const p=person(),profile=p||d;body.append(E('p','am-sub',(p?p.name+' · ':'')+'Benannte E-Mail-Adressen, Rufnummern, Webseiten, Portale und weitere Kontaktwege. Je Art kann ein Weg bevorzugt werden.'));
 for(const type of ['email','phone','mobile','fax'])if(profile[type])info(body,'Standard · '+window.__abContactTools.TYPES[type],profile[type],type==='fax'?'fax':type==='email'?'email':'phone');
 for(const w of profile.contactWays||[]){const s=section(w.label);info(s,window.__abContactTools.TYPES[w.type],w.value,w.type==='email'?'email':w.type==='fax'?'fax':window.__abContactTools.PHONE_TYPES.includes(w.type)?'phone':undefined);if(['website','portal'].includes(w.type)&&window.__abContactTools.safeUrl(w.value)){const link=E('a','am-btn','Öffnen');link.href=window.__abContactTools.safeUrl(w.value);link.target='_blank';link.rel='noopener noreferrer';s.append(link)}if(w.type==='email')s.append(B('E-Mail an diese Adresse',()=>mail(c,p,true,w.value)));s.append(B(w.preferred?'✓ Bevorzugt':'Als bevorzugt verwenden',()=>saveWays(c,p,(profile.contactWays||[]).map(x=>({...x,preferred:x.id===w.id||x.type!==w.type&&x.preferred})))),B('Kontaktweg bearbeiten',()=>editWay(c,p,w)),B('Kontaktweg entfernen',async()=>{if(confirm('Kontaktweg „'+w.label+'“ entfernen?'))await saveWays(c,p,profile.contactWays.filter(x=>x.id!==w.id))},'danger'));body.append(s)}body.append(B('+ Kontaktweg',()=>editWay(c,p,null),'primary'));
}
async function saveWays(c,p,ways){let bundle;
 if(p){if(online())bundle=await request('person',{...ref(c),personId:p.id,baseVersion:M.bundle.personVersions[p.id],patch:window.__abContactTools.withWays({...p,contactWays:ways},p)},'PATCH');else bundle=await savePatch(c,{people:(M.bundle.contact.people||[]).map(x=>x.id===p.id?window.__abContactTools.withWays({...x,contactWays:ways},x):x)})}
 else bundle=await savePatch(c,online()?{contactWays:ways}:window.__abContactTools.withWays({...M.bundle.contact,contactWays:ways},M.bundle.contact));
 await cacheBundle(bundle,ref(c));M.bundle=bundle;window.phase5RenderAddressbookV154();
}
function editWay(c,p,w){const id=w?.id||crypto.randomUUID();let baseline=M.bundle;
 smallForm(w?'Kontaktweg bearbeiten':'Neuer Kontaktweg',[['type','Art',w?.type||'email',Object.entries(window.__abContactTools.TYPES)],['label','Bezeichnung',w?.label||''],['value','Adresse, Rufnummer oder Kennung',w?.value||'']],async v=>{
 const profile=p?(baseline.contact.people||[]).find(x=>x.id===p.id):baseline.contact;if(!profile)throw Error('Der Ansprechpartner wurde entfernt. Bitte die Eingaben prüfen.');const ways=[...(profile.contactWays||[])],i=ways.findIndex(x=>x.id===id),next={...v,id,preferred:!!w?.preferred&&!(w.type!==v.type&&ways.some(x=>x.id!==id&&x.type===v.type&&x.preferred))};if(i<0)ways.push(next);else ways[i]=next;
 let b;if(p)b=online()?await request('person',{...ref(c),personId:p.id,baseVersion:baseline.personVersions[p.id],patch:window.__abContactTools.withWays({...profile,contactWays:ways},profile)},'PATCH'):await savePatch(c,{people:baseline.contact.people.map(x=>x.id===p.id?window.__abContactTools.withWays({...x,contactWays:ways},x):x)});
 else b=online()?await request('contact',{...ref(c),version:baseline.version,patch:{contactWays:ways}},'PATCH'):await savePatch(c,window.__abContactTools.withWays({...profile,contactWays:ways},profile));await cacheBundle(b,ref(c));baseline=M.bundle=b;
 },async()=>{const fresh=await request('contact?'+new URLSearchParams(ref(c))),profile=p?fresh.contact.people?.find(x=>x.id===p.id):fresh.contact;if(!profile)throw Error('Der Ansprechpartner wurde entfernt. Die Eingaben können verworfen werden.');const current=profile.contactWays?.find(x=>x.id===id);if(!confirm('Aktuellen Stand prüfen: '+(current?current.label+' · '+current.value:'Kontaktweg ist noch nicht oder nicht mehr vorhanden.')+'\nIhre Eingaben darauf anwenden?'))return false;baseline=M.bundle=fresh;return true});
}
...
async function drawSync(body,c){
 ...
 const fresh=()=>M.root===root&&M.selected===key&&M.tab==='sync'&&body.isConnected;
 try{
...
function editPerson(c,p){
 ...
 smallForm(p?'Ansprechpartner bearbeiten':'Neuer Ansprechpartner',fields.map(([k,label])=>[k,label,base[k],k==='preferredChannel'?Object.entries(CHANNELS):k==='substitutePersonId'?(c.people||[]).filter(x=>x.id!==id).map(p=>[p.id,p.name]):undefined,true]),async values=>{
  let bundle;if(online())bundle=await request('person',{...ref(c),personId:id,baseVersion,patch:{...base,...values}},'PATCH');else{const people=[...(api().contacts().find(x=>identity(x)===identity(c))?.people||c.people||[])],next={...base,...values,id},i=people.findIndex(x=>x.id===id);if(i>=0)people[i]=next;else people.push(next);bundle=await savePatch(c,{people})}
