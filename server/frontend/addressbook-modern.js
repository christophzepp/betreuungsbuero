...
function render(rows,v){
 ...
 const fragment=document.createDocumentFragment();for(const c of rows.slice(M.page*PAGE_SIZE,(M.page+1)*PAGE_SIZE)){const row=E('article','addressbook-card am-row');row.dataset.abLetter=api().letter(c,v);row.classList.toggle('selected',identity(c)===M.selected);
  ...
  const text=E('span','am-row-copy');text.append(E('strong','',api().title(c,v.nameOrder)),E('span','am-sub',[c.role,api().person(c,v.nameOrder),c.__cases||'',isEnded(c)?'Beendet':''].filter(Boolean).join(' · ')));open.append(icon,text);row.append(open);if(!c.__viewMergeId)row.append(X.star(c));fragment.append(row);
 }
