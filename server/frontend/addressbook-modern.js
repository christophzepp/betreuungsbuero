...
function assign(c){
 const associated=new Set((M.bundle?.associations||[]).map(a=>a.caseId));const entries=[...(DC.enabled()?DC.cases().entries():window.__onlineCaseCache?.entries()||[])].filter(([id])=>!associated.has(id)).map(([id,e])=>[id,e.label||id]);if(!entries.length)throw Error('Der Kontakt ist bereits allen verfügbaren Fällen zugeordnet.');
 ...
 smallForm('Einem weiteren Fall zuordnen',[['targetCaseId','Fall','',entries],['role','Rolle in diesem Fall',c.role],['fileNumber','Aktenzeichen im Zielfall',''],['processNumber','Vorgangsnummer im Zielfall',''],['customerNumber','Kundennummer im Zielfall','']],async(values,s)=>{
  const requestedCaseId=targetCaseId||values.targetCaseId;if(online()){targetCaseId=requestedCaseId;s.form.elements.targetCaseId.setAttribute('aria-readonly','true')}
  const bundle=await request('assign',{...ref(c),...values,targetCaseId:requestedCaseId,assignmentId,targetVersion});targetCaseId=requestedCaseId;s.form.elements.targetCaseId.setAttribute('aria-readonly','true');targetVersion=bundle.assignment.version;await cacheBundle(bundle,ref(c));c=selected()||{...c,...bundle.contact};M.bundle=bundle;
 },async s=>{
