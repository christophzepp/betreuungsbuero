...
function info(parent,label,value,kind){if(!value)return;const wrap=E('div','am-info'),copy=B('Kopieren',()=>navigator.clipboard.writeText(String(value)),'quiet');copy.setAttribute('aria-label',label+' kopieren');const content=E('div');content.append(E('span','am-sub',label));
 if(kind==='address'&&window.__mapAddressLinkHTML){const d=E('div');d.innerHTML=window.__mapAddressLinkHTML(String(value));content.append(d)}else if(kind==='phone'){const a=E('a','',value);a.href='tel:'+String(value).replace(/[^+\d]/g,'');a.onclick=()=>{if(selected())X.used(selected())};content.append(a)}else content.append(E('div','',value));wrap.append(content,copy);parent.append(wrap)}
