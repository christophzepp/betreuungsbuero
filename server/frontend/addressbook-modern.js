...
async function cacheBundle(bundle,r,related=true){
 ...
 const local=M.rows.findIndex(x=>(String(x.id||'')===String(row.id)||!r.id&&r.localKey&&api().key(x)===r.localKey)&&ref(x).scope===r.scope&&ref(x).caseId===r.caseId);if(local>=0){const current=M.rows[local],wasSelected=M.selected===identity(current);M.rows[local]={...current,...row};if(wasSelected)M.selected=identity(M.rows[local])}
