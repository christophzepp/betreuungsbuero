...
function info(parent,label,value,kind){if(!value)return;const wrap=E('div','am-info'),copy=B('Kopieren',()=>navigator.clipboard.writeText(String(value)),'quiet');copy.setAttribute('aria-label',label+' kopieren');const content=E('div'),line=E('div','am-info-value');content.append(E('span','am-sub',label),line);if(['email','phone','fax','file'].includes(kind))line.append(contactIcon(kind));
 if(kind==='address'&&window.__mapAddressLinkHTML){const d=E('div');d.innerHTML=window.__mapAddressLinkHTML(String(value));line.append(d)}else if(kind==='email')line.append(emailLink(value));else if(kind==='fax')line.append(faxLink(value));else if(kind==='phone'){const a=E('a','am-contact-link',value);a.href='tel:'+String(value).replace(/[^+\d]/g,'');a.onclick=()=>{if(selected())X.used(selected())};line.append(a)}else line.append(E('div','',value));wrap.append(content,copy);parent.append(wrap)}
function drawDetail(c){
 if(!c||M.utility)return;const pane=$('.am-detail');pane.classList.remove('am-editing');const scroll=pane.dataset.contact===identity(c)?pane.scrollTop:0;pane.dataset.contact=identity(c);pane.replaceChildren();const top=E('div','am-detail-top');top.append(E('span','am-badge'+(isEnded(c)?' ended':''),c.status||'Aktiv'),B(c.__viewMergeId?'Fallkontakte öffnen':'Bearbeiten',()=>edit(c)));pane.append(top,E('h2','',name(c)),E('p','am-sub',c.role||'Ohne Rolle'));
 X.heading(c,top,pane);
