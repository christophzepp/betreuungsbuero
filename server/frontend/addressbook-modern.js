...
async function request(path,body,method){if(DC.enabled())return DC.request(path,body,method||(body?'POST':'GET'));const r=await fetch('/api/addressbook/'+path,{method:method||(body?'POST':'GET'),credentials:'same-origin',headers:body?{'Content-Type':'application/json'}:{},body:body?JSON.stringify(body):undefined});const j=await r.json().catch(()=>({}));if(!r.ok){const e=new Error(j.error||'Das Adressbuch ist momentan nicht erreichbar.');e.status=r.status;e.code=j.code;e.candidates=j.candidates;throw e}return j}
...
function personal(c,p){const a=(c.addresses||[]).find(a=>a.id===c.preferredAddressId);if(a)c={...c,...Object.fromEntries(['street','house','houseLetter','postal','city','postbox','country'].map(k=>[k,a[k]||'']))};return p?{...c,key:c.key||window.phase5ContactKey?.(c)||api().key(c),salutation:p.salutation||c.salutation,_personId:p.id,firstName:p.name,lastName:'',title:'',email:p.email||c.email,phone:p.phone||c.phone,mobile:p.mobile||c.mobile,fax:p.fax||c.fax,preferredChannel:p.preferredChannel||c.preferredChannel,phoneHours:p.phoneHours||c.phoneHours,absentFrom:p.absentFrom||'',absentUntil:p.absentUntil||'',absenceNote:p.absenceNote||''}:c}
function mount(){
 ...
 const listTools=E('div','am-list-tools');tabs.after(listTools);listTools.append(listmeta,selection);
...
function render(rows,v){
 ...
 if(!rows.length){list.append(E('p','am-empty','Keine Kontakte für diese Filter.'));M.selected='';if(!M.editor&&!M.small&&!M.utility){M.bundle=null;M.load++;M.root.classList.remove('am-show-detail');$('.am-detail').classList.remove('am-editing');$('.am-detail').replaceChildren(E('p','am-empty','Filter anpassen oder einen neuen Kontakt anlegen.'))}return}
 ...
 const fragment=document.createDocumentFragment();for(const c of rows.slice(M.page*PAGE_SIZE,(M.page+1)*PAGE_SIZE)){const row=E('article','addressbook-card am-row');row.dataset.abLetter=api().letter(c,v);row.classList.toggle('selected',identity(c)===M.selected);
  ...
  const open=B('',()=>choose(c));open.className='am-row-open';const icon=X.photo(c,'am-avatar am-row-picture')||E('span','am-avatar',c.institution?'▤':([c.firstName?.[0],c.lastName?.[0]].filter(Boolean).join('')||'○'));
  const text=E('span','am-row-copy');text.append(E('strong','',api().title(c,v.nameOrder)),E('span','am-sub',[c.role,api().person(c,v.nameOrder),c.__cases||'',isEnded(c)?'Beendet':''].filter(Boolean).join(' · ')));open.append(icon,text);row.append(open);const favorite=X.star(c);if(favorite)row.append(favorite);fragment.append(row);
 }
 ...
 if(!M.editor&&!M.small&&!M.utility)drawDetail(selected());
}
...
async function choose(c){if(!await flush())return;closeMenus();M.editRequest++;M.editor=null;M.utility=false;M.selected=identity(c);M.tab='data';M.root.classList.add('am-show-detail');await X.used(c);window.phase5RenderAddressbookV154()}
async function loadDetails(c){
 const token=++M.load;const r=ref(c);if(DC.enabled()){M.bundle=DC.details(r);return M.bundle}if(!online()||!r.id){M.bundle={contact:c,history:c._addressHistory||[],associations:[],communications:localCommunications(c)};return M.bundle}
...
function contactIcon(kind){
 ...
 const svg=document.createElementNS('http://www.w3.org/2000/svg','svg'),path=document.createElementNS(svg.namespaceURI,'path');svg.setAttribute('class','am-icon');svg.setAttribute('viewBox','0 0 24 24');svg.setAttribute('fill','none');svg.setAttribute('stroke','currentColor');svg.setAttribute('stroke-width','1.7');svg.setAttribute('stroke-linecap','round');svg.setAttribute('stroke-linejoin','round');svg.setAttribute('aria-hidden','true');svg.setAttribute('focusable','false');path.setAttribute('d',paths[kind]||paths.file);svg.append(path);return svg;
}
function emailLink(value){const link=E('a','am-contact-link',value);link.href='mailto:'+encodeURIComponent(String(value));link.title='E-Mail intern verfassen';link.onclick=event=>{event.preventDefault();const c=selected();if(c)mail(c,person(),true,value).catch(e=>message(e.message,true))};return link}
function faxLink(value){
 const url=()=>window.__abContactTools.safeUrl(window.getExportGlobal?.()?.simpleFaxComposeUrl||''),link=E('a','am-contact-link',value);
 link.href=url()||'#';link.target='_blank';link.rel='noopener noreferrer';link.title='Neues Fax im Faxdienst öffnen · Nummer zum Einfügen kopieren';link.setAttribute('aria-label','Fax an '+value+' vorbereiten');
 // Den Link direkt öffnen lassen: Ein vorheriges await blockiert Pop-ups in Safari.
 link.onclick=event=>{const target=url();if(!target){event.preventDefault();message('Bitte unter Einstellungen → Versandwege eine gültige Adresse für „Neues Fax“ hinterlegen.',true);return}link.href=target;
  const copied=()=>message('Faxnummer '+value+' kopiert. Bitte im Faxdienst als Empfänger einfügen.'),failed=()=>message('Faxnummer konnte nicht kopiert werden. Bitte '+value+' im Faxdienst als Empfänger eintragen.',true);
  try{navigator.clipboard.writeText(String(value)).then(copied,failed)}catch(_){failed()}if(selected())X.used(selected());
 };return link;
}
function info(parent,label,value,kind){
 if(!value)return;const wrap=E('div','am-info'),copy=B('Kopieren',()=>navigator.clipboard.writeText(String(value)),'quiet');copy.setAttribute('aria-label',label+' kopieren');const symbol=E('span','am-info-symbol'),content=E('div','am-info-content'),line=E('div','am-info-value');symbol.setAttribute('aria-hidden','true');content.append(E('span','am-sub',label),line);
 if(['address','email','phone','fax','file'].includes(kind))symbol.append(contactIcon(kind==='address'?'addresses':kind));
 if(kind==='address'&&window.__mapAddressLinkHTML){const d=E('div');d.innerHTML=window.__mapAddressLinkHTML(String(value));const link=d.querySelector('a');if(link){link.textContent=String(value);link.classList.add('am-contact-link');line.append(link)}else line.textContent=String(value)}
 else if(kind==='email')line.append(emailLink(value));else if(kind==='fax')line.append(faxLink(value));else if(kind==='phone'){const a=E('a','am-contact-link',value);a.href='tel:'+String(value).replace(/[^+\d]/g,'');a.onclick=()=>{if(selected())X.used(selected())};line.append(a)}else line.textContent=String(value);
 wrap.append(symbol,content,copy);parent.append(wrap);
}
function drawDetail(c){
 if(!c||M.utility)return;const pane=$('.am-detail');pane.classList.remove('am-editing');const scroll=pane.dataset.contact===identity(c)?pane.scrollTop:0;pane.dataset.contact=identity(c);pane.replaceChildren();
 const header=E('header','am-contact-header'),top=E('div','am-detail-top'),identityBlock=E('div','am-contact-identity'),summary=E('div','am-contact-summary'),meta=E('div','am-contact-meta');
 meta.append(E('span','am-sub',c.role||'Ohne Rolle'),E('span','am-badge'+(isEnded(c)?' ended':''),c.status||'Aktiv'));summary.append(E('h2','',name(c)),meta);identityBlock.append(summary);const controls=E('div','am-contact-controls');controls.append(B(c.__viewMergeId?'Fallkontakte öffnen':'Bearbeiten',()=>edit(c),'am-contact-edit'));top.append(identityBlock,controls);header.append(top);pane.append(header);
 X.heading(c,top,identityBlock);if(!c.__viewMergeId){const sync=E('div','am-contact-sync');header.append(sync);syncStatus(sync,c)}
 const people=E('div','am-person');if(c.people?.length){const label=E('label','','Ansprechpartner für Dokument, Dokumentation und E-Mail'),select=E('select');select.id='amPerson';select.append(new Option('Institution / allgemeiner Kontakt',''));for(const p of c.people)select.append(new Option([p.name,p.department].filter(Boolean).join(' · '),p.id));const chosen=M.personIds.get(identity(c));select.value=c.people.some(p=>p.id===chosen)?chosen:'';select.onchange=()=>{M.personIds.set(identity(c),select.value);drawTab(c)};label.append(select);people.append(label)}header.append(people);
 const actions=E('div','am-detail-actions am-contact-actions');
 ...
 actions.style.setProperty('--am-action-columns',actions.children.length===4?'1.7fr 1.3fr 1fr 1fr':`repeat(${actions.children.length},minmax(0,1fr))`);header.append(actions);
 ...
 if(!['data','people','cases','communication','history'].includes(M.tab))M.tab='data';
 const tabs=E('nav','am-tabs');tabs.setAttribute('aria-label','Kontaktdetails');for(const [key,label,short] of [['data','Kontaktdaten','Kontakt'],['people','Ansprechpartner','Ansprechpartner'],['cases','Fälle & Standard','Fälle'],['communication','Kommunikation','Kommunikation'],['history','Änderungen','Änderungen']]){if(c.__viewMergeId&&key!=='data')continue;const b=B('',()=>{M.tab=key;drawTab(c);if(innerWidth<=680)requestAnimationFrame(()=>{const pane=$('.am-detail'),tabs=$('.am-tabs');if(pane&&tabs)pane.scrollTop+=tabs.getBoundingClientRect().top-pane.getBoundingClientRect().top})});b.dataset.tab=key;b.setAttribute('aria-label',label);b.title=label;b.append(contactIcon(key),E('span','am-tab-label',short));tabs.append(b)}const caption=E('p','am-tab-caption');caption.setAttribute('aria-hidden','true');pane.append(tabs,caption,E('div','am-tab-body'));
 const more=E('details','am-more');more.append(E('summary','am-btn','Weitere Kontaktaktionen'));const other=E('div','am-detail-actions');
 other.append(B('Daten kopieren',()=>copyContact(c)));if(!c.__viewMergeId)other.append(B(isEnded(c)?'Aktivieren':'Beenden',()=>saveStatus(c,!isEnded(c)),isEnded(c)?'':'warning'),B('Löschen',()=>removeContact(c),'danger'));
...
function drawTab(c){
 const body=$('.am-tab-body');if(!body||M.editor||M.small||M.utility)return;body.replaceChildren();for(const b of M.root.querySelectorAll('[data-tab]')){const active=b.dataset.tab===M.tab;b.setAttribute('aria-pressed',String(active));if(active&&$('.am-tab-caption'))$('.am-tab-caption').textContent=b.getAttribute('aria-label')}
 const more=$('.am-more');if(more){more.hidden=M.tab!=='data';if(more.hidden)more.open=false}
 const data=M.tab==='data'?personal(M.bundle?.contact||c,person()):M.bundle?.contact||c;
 if(M.tab==='data'){
  const s=section(person()?.name||'Kontaktinformationen');info(s,'Anschrift',api().address(data),'address');info(s,'E-Mail',data.email,'email');info(s,'Telefon',data.phone,'phone');info(s,'Mobil',data.mobile,'phone');info(s,'Fax',data.fax,'fax');info(s,'Aktenzeichen',data.fileNumber,'file');info(s,'Vorgangsnummer',data.processNumber);info(s,'Kundennummer',data.customerNumber);info(s,'IBAN',data.iban);info(s,'BIC',data.bic);info(s,'Bank',data.bankName);if(data.note)s.append(E('p','am-note',data.note));if(s.childElementCount>1)body.append(s);if(!c.__viewMergeId)CE.overview(body,c,M.bundle?.contact||c);return;
 }
 ...
 if(M.tab==='people'){
  ...
  const editable=online()||DC.enabled(),associations=editable?M.bundle.associations:localAssociations(c);
  ...
  for(const a of associations){const s=section(a.label);s.append(E('p','am-sub',[a.role,a.fileNumber||'Ohne Aktenzeichen',a.customerNumber?'Kunde '+a.customerNumber:'',a.processNumber,a.status].filter(Boolean).join(' · ')));
   if(!editable){for(const [purpose,pid] of Object.entries(a.standards||{}))s.append(E('p','am-sub','Standard für '+purposeLabel(purpose)+' · '+(pid?(a.people.find(p=>p.id===pid)?.name||'Ansprechpartner'):'Institution')));body.append(s);continue}
...
function drawCommunication(body,c){
 ...
 head.append(E('h3','','Kommunikationsverlauf'),E('p','am-sub','E-Mails, Schreiben, Gesprächsnotizen und vereinbarte Rückmeldungen.'));
 actions.append(B('+ Kommunikation dokumentieren',()=>documentContact(c,person()),'primary'),B('Rückmeldung vereinbaren',()=>followup(c)));view.append(head,actions);body.append(view);
 if(online()){
  const bar=E('div','am-communication-sync'),sync=E('p','am-sub');sync.id='amMailSync';sync.setAttribute('role','status');sync.textContent=(M.mailSync?.key===identity(c)?M.mailSync.text:'')||'Nachrichten werden abgeglichen …';bar.append(sync,B('Nachrichten aktualisieren',()=>syncMail(c,true),'quiet'));view.append(bar);syncMail(c).catch(e=>message(e.message,true));
 }
 const items=M.bundle.communications;
 if(!items.length){const empty=E('div','am-communication-empty');empty.append(E('h3','','Noch keine verknüpfte Kommunikation'),E('p','am-sub','Dokumentieren Sie ein Gespräch oder vereinbaren Sie eine Rückmeldung. Verknüpfte Einträge aus der Falldokumentation erscheinen hier automatisch.'));view.append(empty);return}
 const list=E('div','am-communication-list');view.append(list);
 for(const item of items){
  const card=E('section','am-section am-communication-entry'),meta=E('div','am-communication-meta'),kind=({mail:'E-Mail',document:'Schreiben',followup:'Rückmeldung'})[item.kind]||'Dokumentation';
  meta.append(E('span','am-communication-kind',kind),E('span','am-sub',dateLabel(item.date)));card.append(meta,E('h3','',item.title||kind));
  const context=[item.caseLabel,item.contactType,item.person,item.accountLabel].filter(Boolean);if(context.length)card.append(E('p','am-sub',context.join(' · ')));if(item.text)card.append(E('p','am-note',item.text));
  const links=E('div','am-communication-actions');links.append(B(({mail:'E-Mail öffnen',document:'Versandeintrag öffnen',followup:'Wiedervorlage öffnen'})[item.kind]||'In Falldokumentation öffnen',()=>openCommunication(item)));
  if(item.mail&&item.kind!=='mail')links.append(B('E-Mail öffnen',()=>window.__mxOpenMsg(item.mail.accountId,item.mail.folder,item.mail.uid)));card.append(links);list.append(card);
 }
 if(M.bundle.communicationCursor)view.append(B('Weitere Kommunikation laden',()=>loadMore(c,'communications')));
}
...
function displayValue(v){if(typeof v==='string'&&v.startsWith('data:image/'))return 'Kontaktbild';if(Array.isArray(v)&&v.every(x=>typeof x==='string'))return v.join(', ')||'—';return v==null||v===''?'—':Array.isArray(v)?v.map(x=>[x.label||x.name,...Object.entries(x).filter(([k])=>!['id','label','name'].includes(k)).map(([k,v])=>v?(LABELS[k]||k)+': '+v:'')].filter(Boolean).join(' · ')).join('; '):typeof v==='object'?JSON.stringify(v):String(v)}
...
async function cacheBundle(bundle,r,related=true){
 ...
 const local=M.rows.findIndex(x=>(String(x.id||'')===String(row.id)||!r.id&&r.localKey&&api().key(x)===r.localKey)&&ref(x).scope===r.scope&&ref(x).caseId===r.caseId);if(local>=0){const current=M.rows[local],wasSelected=M.selected===identity(current);M.rows[local]={...current,...row};const oldIdentity=identity(current),newIdentity=identity(M.rows[local]);if(oldIdentity!==newIdentity&&M.personIds.has(oldIdentity)){M.personIds.set(newIdentity,M.personIds.get(oldIdentity));M.personIds.delete(oldIdentity)}if(wasSelected)M.selected=newIdentity}
 if(!online()){X.migrateLocal(r,row);saveState();return}
 // Nach einer zentralen Änderung auch alle sichtbaren Fallprojektionen nachladen (Excel und Empfänger).
 for(const a of related?bundle.associations||[]:[]){if(a.caseId===r.caseId)continue;try{const response=await fetch('/api/cases/'+encodeURIComponent(a.caseId)+'/contacts',{credentials:'same-origin'});if(!response.ok)continue;const j=await response.json(),list=(j.contacts||[]).map(x=>({...x.data,id:x.id}));const cached=window.__onlineCaseCache?.get(a.caseId);if(cached?.data)cached.data.contacts=list;if(a.caseId===activeCase())state.caseData.contacts=list}catch(_e){}}
}
const GROUPS=[['Grunddaten',['_category','status','role','salutation','title','firstName','lastName','institution']],['Anschrift',['street','house','houseLetter','postal','city','postbox','country']],['Kontakt',['phoneArea','phoneNumber','mobileArea','mobileNumber','email','faxArea','faxNumber']],['Referenzen & Bank',['fileNumber','processNumber','customerNumber','iban','bic','bankName']],['Notizen',['note']]];
const LABELS={tags:'Schlagwörter',groups:'Gruppen',customFields:'Eigene Zusatzfelder',imageData:'Kontaktbild',imageKind:'Bildart',_category:'Kategorie',status:'Status',role:'Rolle',salutation:'Anrede',title:'Titel',firstName:'Vorname',lastName:'Nachname',institution:'Institution',street:'Straße',house:'Hausnummer',houseLetter:'Zusatz',postal:'PLZ',city:'Ort',country:'Land',postbox:'Postfach',phone:'Telefon',mobile:'Mobil',fax:'Fax',phoneArea:'Vorwahl Telefon',phoneNumber:'Telefon',mobileArea:'Vorwahl Mobil',mobileNumber:'Mobil',email:'E-Mail',faxArea:'Vorwahl Fax',faxNumber:'Fax',fileNumber:'Aktenzeichen',processNumber:'Vorgangsnummer',customerNumber:'Kundennummer je Fall',contactWays:'Benannte Kontaktwege',addresses:'Benannte Anschriften',preferredAddressId:'Bevorzugte Anschrift',preferredChannel:'Bevorzugter Kontaktweg',phoneHours:'Telefonzeiten',absentFrom:'Abwesend ab',absentUntil:'Abwesend bis',absenceNote:'Hinweis zur Abwesenheit',substituteContactId:'Vertretung',iban:'IBAN',bic:'BIC',bankName:'Bankname',note:'Notizen',people:'Ansprechpartner',_standardRecipients:'Standardempfänger'};
function autoSizeNote(input){
 let appliedHeight=0,manualHeight=0,width=0,frame=0;
 const fit=()=>{frame=0;if(!input.isConnected||!input.clientWidth)return;const current=input.getBoundingClientRect().height;
  // Eine mit dem nativen Ziehgriff gewählte Höhe bleibt beim Weiterschreiben erhalten.
  if(appliedHeight&&Math.abs(current-appliedHeight)>1)manualHeight=current;
  const scroller=input.closest('.am-edit'),scroll=scroller?.scrollTop||0,style=getComputedStyle(input),border=parseFloat(style.borderTopWidth)+parseFloat(style.borderBottomWidth);
  input.style.height='auto';input.style.height=Math.max(Math.ceil(input.scrollHeight+border),manualHeight)+'px';appliedHeight=input.getBoundingClientRect().height;if(scroller)scroller.scrollTop=scroll;
 };
 const schedule=()=>{if(!frame)frame=requestAnimationFrame(fit)};input.addEventListener('input',schedule);
 const observer=new ResizeObserver(()=>{if(!input.isConnected){observer.disconnect();cancelAnimationFrame(frame);return}if(input.clientWidth&&input.clientWidth!==width){width=input.clientWidth;schedule()}});observer.observe(input);fit();
}
async function edit(c,initial={},focusSection=''){M.utility=false;
 ...
 if(online())await window.__baLoadServerBuero?.();if(M.root!==root||!root.isConnected||token!==M.editRequest)return;
 const draft={...structuredClone(bundle.contact),...initial};for(const k of ['phone','mobile','fax'])if(!draft[k+'Number']&&draft[k]){const parts=String(draft[k]).split('/');draft[k+'Area']=parts.length>1?parts.shift():'';draft[k+'Number']=parts.join('/')}
 window.__abModernSetupCombos?.(draft._category);
 M.editor={ref:r,c,bundle,draft,creationId:crypto.randomUUID(),pending:{...initial},saving:null,timer:null,error:false,new:!c};M.root.classList.add('am-show-detail');
 const pane=$('.am-detail');pane.classList.add('am-editing');pane.scrollTop=0;pane.replaceChildren();pane.append(E('h2','',c?'Kontakt bearbeiten':'Neuer Kontakt'),E('p','am-sub',online()?(c?'Änderungen werden automatisch gespeichert.':'Neue Kontakte werden vor der Anlage auf Dubletten geprüft. Danach werden Änderungen automatisch gespeichert.'):'Lokaler Kontakt · Änderungen mit „Fertig“ übernehmen.'));
 const form=E('form','am-edit');form.onsubmit=e=>{e.preventDefault();finishEdit()};
 const duplicates=E('section','am-duplicates');duplicates.hidden=true;pane.append(duplicates);const footer=E('div','am-edit-footer');const conflict=B('Änderungskonflikt auflösen',resolveConflict,'am-conflict',true),retry=B('Speichern erneut versuchen',()=>savePending(),'am-retry',true);conflict.hidden=true;retry.hidden=true;footer.append(B(c?'Fertig':'Kontakt anlegen',finishEdit,'primary'),retry,conflict,B('Ungespeicherte Eingaben verwerfen',discard,'quiet',true));pane.append(form,footer);CE.render(form,M.editor,focusSection);status();if(!c)checkDuplicates(M.editor);
}
function changed(k,v){const e=M.editor;if(!e)return;if(k==='_category')window.__abModernSetupCombos?.(v);e.draft[k]=v;e.pending[k]=v;e.error=false;e.allowDuplicates='';
 ...
 clearTimeout(e.timer);message(online()?'Änderungen noch nicht gespeichert …':'Lokale Änderungen noch nicht übernommen …');if(e.new){message('Neuer Kontakt · Angaben prüfen und Kontakt anlegen');e.timer=setTimeout(()=>checkDuplicates(e),300)}else if(online())e.timer=setTimeout(()=>savePending(true),650);
}
async function savePending(automatic=false){
 const e=M.editor;if(!e)return true;if(e.processing)await e.processing;if(M.editor!==e)return true;clearTimeout(e.timer);if(e.saving){await e.saving;if(M.editor!==e)return true;return e.error?false:savePending(automatic)}
 try{CE.validate(e,!automatic)}catch(err){e.error=true;message(err.message,true);return false}
 ...
 if(e.new&&!online()&&e.allowDuplicates!==draftStamp(e)){await checkDuplicates(e);if(e.duplicates?.length){showDuplicates(e);message('Ähnliche Kontakte vorhanden. Bitte die Vorschläge prüfen.',true);return false}}
 // Bevorzugte Wege und Standardfelder vor dem Speichern in beiden Betriebsarten angleichen.
 try{const normalized=window.__abContactTools.withWays(e.draft,e.bundle.contact);for(const [k,v] of Object.entries(normalized))if(JSON.stringify(v)!==JSON.stringify(e.draft[k])){e.draft[k]=v;e.pending[k]=v}}catch(err){e.error=true;message(err.message,true);return false}
 const sent=structuredClone(e.new?Object.fromEntries(Object.entries(e.draft).filter(([k])=>Object.hasOwn(LABELS,k)||['phone','mobile','fax'].includes(k))):{...e.pending});e.pending={};message('Wird gespeichert …');
 e.saving=(async()=>{
 try{
  let bundle;
  if(online())bundle=await request('contact',{...e.ref,patch:sent,creationId:e.creationId,version:e.bundle.version,allowDuplicates:e.allowDuplicates===draftStamp(e)},e.new?'POST':'PATCH');
  else if(DC.enabled())bundle=await DC.save(e.ref,{...e.draft,id:e.ref.id||crypto.randomUUID()});
  ...
  const wasNew=e.new;e.new=false;e.bundle=bundle;e.error=false;if($('.am-retry'))$('.am-retry').hidden=true;if($('.am-conflict'))$('.am-conflict').hidden=true;
  await cacheBundle(bundle,e.ref);e.ref.id=bundle.contact.id;M.bundle=bundle;const beforeSaveDraft=e.draft;e.draft={...structuredClone(bundle.contact),...e.pending};for(const [key,refresh] of Object.entries(e.refreshCollections||{}))if(!Object.hasOwn(e.pending,key)&&JSON.stringify(beforeSaveDraft[key])!==JSON.stringify(e.draft[key]))refresh();for(const input of M.root.querySelectorAll('.am-edit [id^=amEdit_]')){const key=input.name;if(!Object.hasOwn(e.pending,key)&&input.tagName!=='TEXTAREA')input.value=e.draft[key]||''}
  ...
  if(!Object.keys(e.pending).length)message(online()?'Gespeichert · '+new Date().toLocaleTimeString('de-DE',{hour:'2-digit',minute:'2-digit'}):DC.enabled()?'In der Demo-Sitzung übernommen':'Lokal übernommen · Excel zur zusätzlichen Sicherung exportieren');return true;
 }catch(err){e.pending={...sent,...e.pending};e.error=true;if(err.code==='duplicates'){e.duplicates=err.candidates;showDuplicates(e)}const conflict=$('.am-conflict');if(conflict)conflict.hidden=err.status!==409||err.code==='duplicates';const retry=$('.am-retry');if(retry)retry.hidden=err.status===409;message(err.message,true);return false}
 finally{e.saving=null}
 })();const ok=await e.saving;if(ok&&Object.keys(e.pending).length)return savePending(automatic);return ok;
}
function smallDirty(){return !!M.small&&JSON.stringify(Object.fromEntries(new FormData(M.small.form)))!==M.small.initial}
async function flush(){if(M.small){if(M.small.manual&&!M.small.everSaved){if(M.small.form.elements.targetCaseId?.value){message('Bitte die neue Fallverknüpfung mit „Fertig“ anlegen oder die Eingaben verwerfen.',true);return false}M.small=null}else if(!await saveSmall())return false;if(!M.small)return !M.editor||await savePending();clearTimeout(M.small.timer);M.small=null}return !M.editor||await savePending()}
async function resolveConflict(){const e=M.editor;if(!e||e.new)return;const fresh=await request('contact?'+new URLSearchParams(e.ref));const differences=Object.keys(e.pending).filter(k=>JSON.stringify(e.bundle.contact[k])!==JSON.stringify(fresh.contact[k])).map(k=>(LABELS[k]||k)+': gespeichert „'+displayValue(fresh.contact[k])+'“ / Ihre Eingabe „'+displayValue(e.pending[k])+'“');if(!confirm('Aktuellen Stand laden und Ihre noch offenen Eingaben darauf anwenden?'+(differences.length?'\n\nDiese Felder würden überschrieben:\n'+differences.join('\n'):' Andere zwischenzeitliche Änderungen bleiben erhalten.')))return;for(const key of Object.keys(e.pending))e.pending[key]=CE.merge(e.bundle.contact[key],e.pending[key],fresh.contact[key]);e.bundle=fresh;e.draft={...fresh.contact,...e.pending};e.render();e.error=false;$('.am-conflict').hidden=true;await savePending()}
async function finishEdit(){if(!await flush())return;M.editor=null;M.tab='data';window.showImportedAddressbook();if(M.rows.length)M.root?.classList.add('am-show-detail')}
async function discard(){const e=M.editor;if(!e)return;if(e.processing)await e.processing;if(e.saving)await e.saving;if((Object.keys(e.pending).length||Object.values(e.unfinishedLabels||{}).some(v=>v.trim()))&&!confirm('Noch nicht gespeicherte Eingaben verwerfen? Bereits automatisch gespeicherte Änderungen bleiben erhalten.'))return;clearTimeout(e.timer);M.editor=null;window.showImportedAddressbook()}
async function savePatch(c,patch){if(c.__viewMergeId)throw Error('Zusammengeführte Ansichten bitte in den zugehörigen Fallkontakten bearbeiten.');const r=ref(c);let bundle;if(online()){const fresh=await request('contact?'+new URLSearchParams(r));bundle=await request('contact',{...r,patch,version:fresh.version},'PATCH')}else if(DC.enabled())bundle=await DC.save(r,{...DC.find(r),...patch});else{const next={...c,...patch,id:c.id||crypto.randomUUID(),_pendingWrite:true};next._addressHistory=localHistory(c,next);bundle={contact:next,associations:[],history:next._addressHistory,communications:localCommunications(c)};if(r.scope==='office')await window.__baModernSaveLocal?.(next)}await cacheBundle(bundle,r);M.bundle=bundle;window.phase5RenderAddressbookV154();return bundle}
async function saveStatus(c,end){if(end&&!confirm('Kontakt „'+name(c)+'“ als beendet markieren? Er wird unter „Beendete“ geführt und kann jederzeit wieder aktiviert werden.'))return;await savePatch(c,{status:end?'Beendet':'Aktiv'})}
async function removeContact(c){if(!confirm('Kontakt „'+name(c)+'“ in den Papierkorb verschieben?'+(c.__buero?' Seine Fallzuordnungen werden ebenfalls in den Papierkorb verschoben.':'')))return;const r=ref(c);
 if(online()){await request('trash',{...r,version:M.bundle?.version});if(r.scope==='office'){for(const a of M.bundle?.associations||[])await refreshCase(a.caseId);await window.__baLoadServerBuero?.()}}
 else{state.ui.addressbookTrash=state.ui.addressbookTrash||[];state.ui.addressbookTrash.unshift({id:crypto.randomUUID(),label:name(c),ref:r,contact:{...structuredClone(c),id:c.id||crypto.randomUUID()},deletedAt:new Date().toISOString(),actor:'Lokal'});saveState()}
 if(r.scope==='office'){if(!online())await window.__baModernDelete?.(r.id)}else{for(const cd of [r.caseId===activeCase()?state.caseData:null,window.__onlineCaseCache?.get(r.caseId)?.data].filter(Boolean))cd.contacts=(cd.contacts||[]).filter(x=>r.id?x.id!==r.id:api().key(x)!==api().key(c));if(!online())saveState()}
...
async function useDocument(c,p){X.used(c);const k=c.key||window.phase5ContactKey?.(c)||api().key(c);window.phase5ApplyContact(k);{const eo=window.getExportOptions?.(currentReport);if(eo){state.ui.exportOptions[currentReport]={...(state.ui.exportOptions[currentReport]||{}),recipientPersonId:p?.id||'',recipientEmail:p?.email||c.email||''};saveState();window.renderReport?.()}}}
async function mail(c,p,internal,emailOverride){X.used(c);const email=emailOverride||p?.email||c.email;if(!email)throw Error('Für diesen Kontakt ist keine E-Mail-Adresse hinterlegt.');const target={name:p?.name||name(c),email,subject:['Betreuungsangelegenheit',c.fileNumber].filter(Boolean).join(' '),contactLink:currentLink(c,p)};target.contactLink.snapshot.email=email;
...
async function documentContact(c,p){X.used(c);const r=ref(c),id=r.scope==='case'?r.caseId:activeCase();if(!id)throw Error('Bitte zuerst einen Fall öffnen.');M.editor=null;window.openDokuEntryForm(id,-1);window.__fdKontaktUebernehmen?.(personal(c,p),r.scope,id)}
...
async function saveSmall(){
 const s=M.small;if(!s)return true;if(s.processing)await s.processing;if(M.small!==s)return true;clearTimeout(s.timer);if(s.saving){await s.saving;return s.error?false:saveSmall()}
 if(s.conflict)return false;if(!smallDirty())return true;
 try{s.validate?.()}catch(err){s.error=true;s.errorNode.textContent=err.message;message('Noch nicht gespeichert · '+err.message,true);return false}
...
function smallForm(title,fields,submit,resolve){
 ...
 s.retry=B('Speichern erneut versuchen',saveSmall,'',true);s.retry.hidden=true;s.resolveButton=B('Aktuellen Stand prüfen',async()=>{if(resolve&&await resolve(s)){s.conflict=false;s.resolveButton.hidden=true;if(s.manual){s.errorNode.textContent='';message('Aktuelle Angaben geladen · bitte prüfen und mit „Fertig“ verknüpfen.')}else await saveSmall()}},'',true);s.resolveButton.hidden=true;
 ...
 form.oninput=()=>{clearTimeout(s.timer);message('Änderungen noch nicht gespeichert …');if(online()&&!s.manual)s.timer=setTimeout(saveSmall,650)};form.onchange=form.oninput;
...
const CHANNELS={...window.__abContactTools.TYPES,post:'Post'};
...
window.__abValidateImport=function(c){
 ...
 if(c.contactWays!=null)window.__abContactTools.ways(c.contactWays);for(const p of c.people||[])if(p.contactWays!=null)window.__abContactTools.ways(p.contactWays);Object.assign(c,window.__abExtrasData.clean(c));dates(c);return c;
};
function draftStamp(e){return JSON.stringify(e.draft)}
async function checkDuplicates(e){
 if(!e||M.editor!==e||!e.new)return;const stamp=draftStamp(e);let candidates;
 try{candidates=online()?(await request('duplicates',{...e.ref,patch:e.draft})).candidates:window.__abContactTools.duplicates(e.draft,api().contacts().map(c=>({...ref(c),contact:c}))).slice(0,12)}catch(err){if(M.editor===e)message('Dublettenprüfung fehlgeschlagen: '+err.message,true);return}
 if(M.editor!==e||draftStamp(e)!==stamp)return;e.duplicates=candidates;showDuplicates(e);
}
function showDuplicates(e){const box=$('.am-duplicates');if(!box||M.editor!==e)return;box.replaceChildren();box.hidden=!e.duplicates?.length;if(box.hidden)return;box.append(E('h3','','Ähnliche Kontakte vorhanden'),E('p','am-sub','Bitte prüfen, ob einer dieser Kontakte gemeint ist. Eine gemeinsame Telefonnummer kann zu mehreren Ansprechpartnern gehören.'));
 for(const r of e.duplicates){const row=section(r.label||name(r.contact));row.append(E('p','am-sub',[r.caseLabel,...r.reasons].filter(Boolean).join(' · ')),B('Vorhandenen Kontakt verwenden',()=>useDuplicate(e,r),'',true));box.append(row)}
 box.append(B('Bewusst als neuen Kontakt anlegen',async()=>{e.allowDuplicates=draftStamp(e);if(await savePending())await finishEdit()},'quiet',true));
}
async function useDuplicate(e,r){if(M.editor!==e)return;let target=r;
 if(online()){
  if(e.ref.scope==='case'&&(r.scope!=='case'||r.caseId!==e.ref.caseId)){const b=await request('assign',{...r,targetCaseId:e.ref.caseId,role:e.draft.role||'',fileNumber:e.draft.fileNumber||'',processNumber:e.draft.processNumber||'',customerNumber:e.draft.customerNumber||''});target={scope:'case',caseId:e.ref.caseId,id:b.assignment.contactId};await refreshCase(target.caseId)}
  else if(e.ref.scope==='office'&&r.scope==='case'){const b=await request('assign',{...r,targetCaseId:r.caseId});target={scope:'office',caseId:'',id:b.centralId}}
 }
 clearTimeout(e.timer);e.pending={};M.editor=null;if(online())await window.__abOpenContact(target.id,target.scope==='office'?'':target.caseId);else{const existing=api().contacts().find(c=>identity(c)===JSON.stringify([r.scope,r.caseId,r.id||r.localKey]));if(!existing)throw Error('Kontakt nicht mehr verfügbar.');await choose(existing)};
}
async function showTrash(){M.utility=true;
 const root=M.root,pane=$('.am-detail'),token=++M.load;M.editor=null;M.small=null;M.root.classList.add('am-show-detail');pane.scrollTop=0;pane.classList.remove('am-editing');pane.replaceChildren(B('← Kontakte',()=>window.showImportedAddressbook(),'am-utility-back quiet'),E('h2','','Papierkorb'),E('p','am-sub','Gelöschte Kontakte lassen sich mit ihren ursprünglichen Verknüpfungen wiederherstellen. Synchronisationen bleiben anschließend pausiert.'));
 const result=online()?await request('trash'):{items:state.ui.addressbookTrash||[]};if(M.root!==root||token!==M.load||!M.utility)return;
 if(!result.items.length)pane.append(E('p','am-empty','Der Papierkorb ist leer.'));
 X.trash(pane,result.items);for(const item of result.items){const s=section(item.label);s.append(B('Endgültig löschen',()=>X.purge([item]),'danger'));s.append(E('p','am-sub',[dateLabel(item.deletedAt),item.actor,...(item.cases||[]).map(x=>x.label)].filter(Boolean).join(' · ')),B('Wiederherstellen',async()=>{
  if(online()){const r=await request('trash/restore',{id:item.id});await window.__baLoadServerBuero?.();for(const c of item.cases||[])await refreshCase(c.id);await window.__abOpenContact(r.target.id,r.target.scope==='office'?'':r.target.caseId);message(r.warnings?.length?r.warnings.join(' '):'Kontakt wiederhergestellt')}
  else{if(item.ref.scope==='office')await window.__baModernSaveLocal?.(item.contact);else{const cd=item.ref.caseId===activeCase()?state.caseData:window.__onlineCaseCache?.get(item.ref.caseId)?.data;if(!cd)throw Error('Bitte den ursprünglichen Fall öffnen.');if((cd.contacts||[]).some(x=>x.id===item.contact.id))throw Error('Diese Kontakt-ID ist bereits vorhanden.');(cd.contacts||=[]).push(item.contact)}X.migrateLocal(item.ref,item.contact);state.ui.addressbookTrash=state.ui.addressbookTrash.filter(x=>x.id!==item.id);saveState();window.showImportedAddressbook();message('Kontakt wiederhergestellt')}
 },'primary'));pane.append(s)}pane.append(B('Zurück zum Adressbuch',()=>window.showImportedAddressbook()));
}
function signatureForm(text=''){M.utility=true;
 const c=selected(),pane=$('.am-detail');M.editor=null;M.small=null;M.load++;M.root.classList.add('am-show-detail');pane.classList.remove('am-editing');pane.replaceChildren(E('h2','','Kontakt aus Signatur'),E('p','am-sub','Signaturtext einfügen oder aus einer geöffneten E-Mail übernehmen. Erkannte Angaben werden vor dem Speichern zur Prüfung angezeigt.'));
 const label=E('label','','Signaturtext'),input=E('textarea');input.id='amSignatureText';input.maxLength=50000;input.rows=9;input.value=text;label.append(input);pane.append(label,B('Vorschlag prüfen',()=>signaturePreview(window.__abContactTools.signature(input.value),c,input.value),'primary'),B('Abbrechen',()=>window.showImportedAddressbook()));
}
async function signaturePreview(proposal,c,text){
 const token=++M.load,pane=$('.am-detail');pane.scrollTop=0;pane.replaceChildren(E('h2','','Erkannte Angaben prüfen'),E('p','am-sub','Die Erkennung macht Vorschläge. Angaben können korrigiert oder abgewählt werden. Bei einem vorhandenen Kontakt sind nur leere Felder vorausgewählt.'));
 const base=c?(online()?(await request('contact?'+new URLSearchParams(ref(c)))):{contact:c}):null;if(token!==M.load||!pane.isConnected||!M.utility)return;const form=E('form','am-edit'),checks=new Map();
 for(const k of ['institution','title','firstName','lastName','email','phone','mobile','fax','street','house','houseLetter','postal','city']){const row=E('div','am-signature-row'),check=E('input');check.type='checkbox';check.checked=!!proposal[k]&&!base?.contact?.[k];check.setAttribute('aria-label',LABELS[k]+' übernehmen');const label=E('label','',LABELS[k]),input=E('input');input.value=proposal[k]||'';input.name=k;label.append(input);if(base?.contact?.[k])label.append(E('small','am-sub','Bisher: '+base.contact[k]));row.append(check,label);form.append(row);checks.set(k,{check,input})}
 const extraWays=[];for(const way of proposal.contactWays||[]){if(checks.has(way.type)&&proposal[way.type]===way.value)continue;const row=E('div','am-signature-row'),check=E('input'),label=E('label','',way.label+' · '+window.__abContactTools.TYPES[way.type]),input=E('input');check.type='checkbox';check.checked=!base?.contact.contactWays?.some(w=>w.type===way.type&&w.value===way.value);check.setAttribute('aria-label',way.label+' übernehmen');input.value=way.value;label.append(input);row.append(check,label);form.append(row);extraWays.push({way,check,input})}
 const selectedWays=patch=>window.__abContactTools.ways([...(proposal.contactWays||[]).filter(w=>checks.has(w.type)&&proposal[w.type]===w.value&&patch[w.type]).map(w=>({...w,value:patch[w.type],id:crypto.randomUUID()})),...extraWays.filter(x=>x.check.checked).map(x=>({...x.way,value:x.input.value.trim(),id:crypto.randomUUID()}))]);
 const values=all=>Object.fromEntries([...checks].filter(([,v])=>all?v.input.value:v.check.checked).map(([k,v])=>[k,v.input.value.trim()]));
 pane.append(form,B('Als neuen Kontakt prüfen',async()=>{const patch=values(false);if(!c)Object.assign(patch,values(false));patch.contactWays=selectedWays(patch);await edit(null,patch);await checkDuplicates(M.editor)},'primary'));
 if(c)pane.append(B('Ausgewählten Kontakt ergänzen',async()=>{const valuesPatch=values(false),patch=window.__abContactTools.signaturePatch(base.contact,valuesPatch,selectedWays(valuesPatch));if(!Object.keys(patch).length)throw Error('Bitte mindestens eine Angabe auswählen.');let bundle;if(online())bundle=await request('contact',{...ref(c),version:base.version,patch},'PATCH');else bundle=await savePatch(c,patch);await cacheBundle(bundle,ref(c));window.showImportedAddressbook();message('Ausgewählte Angaben übernommen')}));
 pane.append(B('Zurück zur Signatur',()=>signatureForm(text)),B('Abbrechen',()=>window.showImportedAddressbook()));
}
window.__abSignatureFromMessage=function(m){
 let text=String(m.text||'');if(!text&&m.html){const template=document.createElement('template');template.innerHTML=String(m.html).replace(/<br\s*\/?\s*>|<\/(?:p|div|tr)>/gi,'\n');template.content.querySelectorAll('script,style,iframe,object,blockquote,.gmail_quote').forEach(x=>x.remove());text=template.content.textContent||''}
 return window.__abImportSignature(text);
};
window.__abImportSignature=async function(text){if(!await flush())return;if(!visible())await window.__abSwitchCase(activeCase()||'__buero');signatureForm(String(text||''))};
async function syncStatus(box,c){
 if(!online())return;
 const text=E('span','am-sub','Synchronisation wird geprüft …');box.append(text,B('Synchronisation verwalten',()=>openSync(c),'quiet'));
 try{const result=await request('sync?'+new URLSearchParams(ref(c)));if(!box.isConnected)return;const bindings=result.bindings||[],issues=bindings.filter(b=>b.conflict||b.error||['missing','uncertain','error','conflict'].includes(b.status));text.textContent=issues.length?'Synchronisation: '+issues.length+' Verbindung(en) prüfen':bindings.length?bindings.some(b=>b.enabled)?'Synchronisation aktiv':'Synchronisation pausiert':'Keine Kontaktsynchronisation';text.classList.toggle('am-error',!!issues.length)}catch(e){if(box.isConnected){text.textContent='Synchronisation konnte nicht geprüft werden';text.classList.add('am-error')}}
}
async function openSync(c){
 if(!c||!await flush())return;closeMenus();M.syncDialog?.dialog.close();
 const dialog=E('dialog','am-sync-dialog'),head=E('header','am-sync-dialog-head'),body=E('div','am-sync-dialog-body');head.append(E('h2','','Synchronisation'),B('Schließen',()=>dialog.close()));dialog.append(head,E('p','am-sub',name(c)),body);M.root.append(dialog);M.syncDialog={dialog,body};
 dialog.addEventListener('close',()=>{if(M.syncDialog?.dialog===dialog)M.syncDialog=null;dialog.remove();const status=$('.am-contact-sync');if(status&&M.selected===identity(c)){status.replaceChildren();syncStatus(status,c)}});dialog.showModal();await drawSync(body,c);
}
async function drawSync(body,c){
 if(!online()){body.append(E('p','am-sub','Automatische Kontaktsynchronisation steht im Online-Modus zur Verfügung.'));return}
 const loading=E('p','am-sub','Verbindungen werden geladen …');body.append(loading);const key=identity(c),root=M.root;
 const fresh=()=>M.root===root&&M.selected===key&&M.syncDialog?.body===body&&body.isConnected;
 try{
  const data=await request('sync?'+new URLSearchParams(ref(c)));if(!fresh())return;body.replaceChildren(E('p','am-sub','Gemeinsame Kontaktdaten werden mit dem gewählten Online-Kontakt abgeglichen. Fallbezogene Referenzen und Notizen bleiben hier. Der automatische Abgleich läuft, solange der Server läuft; gleichzeitige Änderungen werden zur Entscheidung angezeigt.'));
  const labels={paused:'Abgleich pausiert',pending:'Bereit zum ersten Abgleich',synced:'Abgeglichen',conflict:'Entscheidung erforderlich',missing:'Online-Kontakt fehlt',uncertain:'Online-Anlage wird geprüft',creating:'Online-Kontakt wird angelegt',error:'Abgleich prüfen',trash:'Aus dem Papierkorb wiederhergestellt · Abgleich pausiert'};
  const reload=async()=>{if(fresh()){if(ref(c).caseId)await refreshCase(ref(c).caseId);if(c.__buero)await window.__baLoadServerBuero?.();M.bundle=await request('contact?'+new URLSearchParams(ref(c)));await cacheBundle(M.bundle,ref(c));await drawSync(body,c)}};
  for(const b of data.bindings){const box=section(b.label+' · '+b.book);box.append(E('p','am-sub',(b.enabled?'Automatisch aktiv':'Pausiert')+' · '+(labels[b.status]||b.status)),E('p','am-sub',b.lastSyncedAt?'Zuletzt: '+dateLabel(b.lastSyncedAt):'Noch kein vollständiger Abgleich'));if(b.error)box.append(E('p','am-sync-error',b.error));
   if(b.conflict){const choices={};for(const field of b.conflict.fields){const row=E('div','am-sync-choice');row.append(E('strong','',LABELS[field]||field));for(const side of ['local','remote']){const label=E('label'),radio=E('input');radio.type='radio';radio.name='sync-'+b.id+'-'+field;radio.value=side;radio.onchange=()=>choices[field]=side;const value=E('span','', (side==='local'?'Hier: ':'Online: ')+displayValue(b.conflict[side][field]));label.append(radio,value);row.append(label)}box.append(row)}if(b.canManage&&b.enabled)box.append(B('Auswahl übernehmen und abgleichen',async()=>{if(b.conflict.fields.some(f=>!choices[f]))throw Error('Bitte für jedes abweichende Feld einen Stand auswählen.');await request('sync/run',{bindingId:b.id,version:b.version,resolution:choices});await reload()},'primary'))}
   if(b.canManage){if(b.enabled)box.append(B('Jetzt abgleichen',async()=>{await request('sync/run',{bindingId:b.id,version:b.version});await reload()}));box.append(B(b.enabled?'Abgleich pausieren':'Automatik aktivieren',async()=>{await request('sync',{bindingId:b.id,version:b.version,enabled:!b.enabled},'PATCH');await reload()}),B('Verknüpfung lösen',async()=>{if(confirm('Nur die Synchronisationsverknüpfung lösen? Der Kontakt bleibt hier und beim Online-Anbieter erhalten.')){await request('sync',{bindingId:b.id,version:b.version},'DELETE');await reload()}},'quiet'))}body.append(box);
  }
  if(!data.sources.length){body.append(E('p','am-sub','Noch keine geeignete Verbindung verfügbar. Microsoft, Google, Nextcloud oder iCloud zunächst in den bestehenden Verbindungs-Einstellungen einrichten.'));return}
  const box=section('Online-Kontakt verknüpfen'),source=E('select'),book=E('select'),remote=E('select'),search=E('input');search.type='search';search.placeholder='Online-Kontakte filtern';search.setAttribute('aria-label','Online-Kontakte filtern');let contacts=[],loaded=false;
  function field(label,input){const l=E('label','',label);l.append(input);box.append(l)}
  source.id='amSyncSource';book.id='amSyncBook';remote.id='amSyncRemote';for(const s of data.sources)source.append(new Option(s.label,s.id));
  function renderOptions(){const previous=remote.value;remote.replaceChildren(new Option('Bitte Online-Kontakt wählen …',''),new Option('Neuen Online-Kontakt anlegen','__new__'));for(const r of contacts.filter(r=>(r.label+' '+r.email).toLowerCase().includes(search.value.toLowerCase())))remote.append(new Option(r.label+(r.email?' · '+r.email:''),r.uid));if([...remote.options].some(o=>o.value===previous))remote.value=previous}
  function chooseBook(){contacts=[];loaded=false;renderOptions();remote.disabled=true;search.value=''}
  function chooseSource(){book.replaceChildren(...data.sources.find(s=>s.id===source.value).books.map(b=>new Option(b.label,b.ref)));chooseBook()}
  source.onchange=chooseSource;book.onchange=chooseBook;search.oninput=renderOptions;chooseSource();field('Verbindung',source);field('Adressbuch',book);
  box.append(B('Online-Kontakte laden',async()=>{const sourceId=source.value,bookRef=book.value;const result=await request('sync/remote',{...ref(c),connectionId:sourceId,book:bookRef});if(!fresh()||source.value!==sourceId||book.value!==bookRef)return;contacts=result.contacts;loaded=true;remote.disabled=false;renderOptions()}));field('Suchen',search);field('Online-Kontakt',remote);
  box.append(E('p','am-sub','Vor einer neuen Anlage vorhandene Online-Kontakte prüfen. Bei der Verknüpfung bestehender Kontakte werden abweichende Angaben zuerst verglichen.'),B('Automatischen Abgleich einrichten',async()=>{if(!loaded||!remote.value)throw Error('Bitte die Online-Kontakte laden und ein Ziel auswählen.');const result=await request('sync/link',{...ref(c),connectionId:source.value,book:book.value,uid:remote.value==='__new__'?'':remote.value});await request('sync/run',{bindingId:result.id});await reload()},'primary'));body.append(box);
 }catch(e){if(fresh()){loading.remove();body.append(E('p','am-sync-error',e.message),B('Erneut laden',()=>drawSync(body,c)))}}
}

const VIEW_IDS={quick:'amQuickFilter',tag:'amTagFilter',group:'amGroupFilter',query:'phase5AddressSearchV154',status:'phase5AddressStatusV154',city:'phase5AddressCityV154',role:'phase5AddressRoleV154',institution:'phase5AddressInstitutionV154',kind:'phase5AddressKindV154',sortBy:'phase5AddressSortV154',direction:'phase5AddressDirectionV154',nameOrder:'phase5AddressNameOrderV154'};
function viewMenu(){
...
function drawAvailability(body,c,d){
 const view=detailView(body,'Erreichbarkeit','Bevorzugte Kontaktwege, Telefonzeiten, Abwesenheiten und Vertretung.','am-availability'),actions=E('div','am-view-actions');actions.append(B('Erreichbarkeit bearbeiten',()=>edit(c,{},'availability'),'quiet'));view.append(actions);
...
function editAssociation(c,a){let version=a.version;const target={scope:'case',caseId:a.caseId,id:a.contactId,localKey:a.localKey||''};smallForm('Fallangaben: '+a.label,['role','fileNumber','processNumber','customerNumber'].map(k=>[k,LABELS[k],a[k]]),async values=>{const bundle=await request('contact',{...target,version,patch:values},'PATCH');version=bundle.version;await cacheBundle(bundle,target);target.id=bundle.contact.id;target.localKey='';c=selected()||c;M.bundle=await request('contact?'+new URLSearchParams(ref(c)))},async s=>{const fresh=await request('contact?'+new URLSearchParams(target));if(!await rebaseSmall(s,fresh.contact,'Aktuelle Fallangaben'))return false;version=fresh.version;return true})}
...
function editPerson(c,p){
 ...
 const formState=smallForm(p?'Ansprechpartner bearbeiten':'Neuer Ansprechpartner',fields.map(([k,label])=>[k,label,base[k],k==='preferredChannel'?Object.entries(CHANNELS):k==='substitutePersonId'?(c.people||[]).filter(x=>x.id!==id).map(p=>[p.id,p.name]):undefined,true]),async values=>{
  values={...values,contactWays:JSON.parse(values.contactWays||'[]')};let bundle;if(online())bundle=await request('person',{...ref(c),personId:id,baseVersion,patch:{...base,...values}},'PATCH');else{const people=[...(api().contacts().find(x=>identity(x)===identity(c))?.people||c.people||[])],next=window.__abContactTools.withWays({...base,...values,id},base),i=people.findIndex(x=>x.id===id);if(i>=0)people[i]=next;else people.push(next);bundle=await savePatch(c,{people})}
  base=bundle.contact.people.find(x=>x.id===id)||{};baseVersion=bundle.personVersions?.[id]||'';await cacheBundle(bundle,ref(c));M.bundle=bundle;for(const [key] of fields){const input=formState.form.elements[key];if(input.value===(values[key]||''))input.value=base[key]||''}if(waysValue.value===JSON.stringify(values.contactWays)&&waysValue.value!==JSON.stringify(base.contactWays||[])){waysValue.value=JSON.stringify(base.contactWays||[]);renderWays()}
 },async s=>{
  ...
  const ownWays=JSON.parse(own.contactWays||'[]'),waysChanged=JSON.stringify(ownWays)!==JSON.stringify(base.contactWays||[]);
  if(!confirm('Aktuellen Ansprechpartner laden und Ihre Änderungen darauf anwenden? Andere Felder bleiben erhalten.\n\n'+difference))return false;
  const mergedWays=waysChanged?CE.merge(base.contactWays||[],ownWays,remote.contactWays||[]):remote.contactWays||[];base=remote;baseVersion=fresh.personVersions?.[id]||'';for(const [k] of fields)s.form.elements[k].value=changed[k]??remote[k]??'';waysValue.value=JSON.stringify(mergedWays);renderWays();return true;
 });
 const waysValue=E('input');waysValue.type='hidden';waysValue.name='contactWays';waysValue.value=JSON.stringify(base.contactWays||[]);formState.form.append(waysValue);
 const waysSection=section('Benannte Kontaktwege');formState.form.querySelector('.am-small-footer').before(waysSection);const renderWays=CE.ways(waysSection,()=>JSON.parse(waysValue.value),ways=>{waysValue.value=JSON.stringify(ways);waysValue.dispatchEvent(new Event('input',{bubbles:true}))});
 formState.validate=()=>window.__abContactTools.ways(JSON.parse(waysValue.value));formState.initial=JSON.stringify(smallValues(formState));
}
...
/* ADDRESSBOOK-ASSIGNMENT-UI */

async function detach(c,a){if(!confirm('Die zentrale Verknüpfung für „'+a.label+'“ lösen? Der Kontakt bleibt als eigenständiger Fallkontakt erhalten.'))return;const bundle=await request('detach',{...ref(c),targetCaseId:a.caseId,contactId:a.contactId,localKey:a.localKey||''});await cacheBundle(bundle,ref(c));await refreshCase(a.caseId);if(M.selected===identity(c)){M.bundle=bundle;drawTab(c)}}
async function setStandard(c,a,purpose,remove){const p=person(),wasSelected=M.selected===identity(c);const bundle=await request('standard',{caseId:a.caseId,id:a.contactId,localKey:a.localKey||'',purpose,personId:p?.id||'',remove});await cacheBundle(bundle,{scope:'case',caseId:a.caseId,id:a.contactId,localKey:a.localKey||''});await refreshCase(a.caseId);if(wasSelected){c=selected()||c;const fresh=await loadDetails(c);if(fresh)drawTab(c)}}
async function restore(c,h){if(!confirm('Die angezeigte Änderung rückgängig machen? Dies wird als neue Änderung protokolliert.'))return;if(!online()){const patch={};for(const [k,v] of Object.entries(h.changes))if(Object.hasOwn(LABELS,k)&&k!=='_standardRecipients')patch[k]=v.before??(['people','addresses','contactWays','tags','groups','customFields'].includes(k)?[]:'');if(patch.addresses&&(patch.preferredAddressId??c.preferredAddressId)&&!patch.addresses.some(a=>a.id===(patch.preferredAddressId??c.preferredAddressId)))patch.preferredAddressId='';if(!(patch.institution??c.institution)&&!(patch.lastName??c.lastName))throw Error('Die Anlage eines Kontakts lässt sich hier nicht rückgängig machen.');await savePatch(c,patch);return}const bundle=await request('restore',{...ref(c),historyId:h.id,version:M.bundle.version});await cacheBundle(bundle,ref(c));M.bundle=bundle;window.phase5RenderAddressbookV154()}
...
window.__abOpenContact=async(id,caseId)=>{await window.__abSwitchCase(caseId||'__buero');if(caseId&&String(caseId)!==activeCase())throw Error('Der verknüpfte Fall konnte nicht geöffnet werden.');if(!visible())window.showImportedAddressbook();const c=api().contacts().find(c=>c.id===id);if(c){if(!M.rows.some(x=>identity(x)===identity(c))){for(const id of ['phase5AddressSearchV154','phase5AddressCityV154','phase5AddressRoleV154','phase5AddressInstitutionV154','amQuickFilter','amTagFilter','amGroupFilter']){const el=$('#'+id);if(el)el.value=''}$('#phase5AddressStatusV154').value='all';$('#phase5AddressKindV154').value='all';$('#amMissingEmail').checked=false}await choose(c)}else throw Error('Kontakt nicht mehr verfügbar.')};
const visible=()=>!!M.root?.isConnected&&!document.getElementById('modal')?.classList.contains('hidden');
window.__abModern={extraFilter:(rows,v)=>X.filter(rows,v),mount,render,jumpToLetter,active:visible,hold:()=>!!(visible()&&(M.editor||M.small)),selected,flush,edit,savePatch,refresh:()=>{if(visible())window.phase5RenderAddressbookV154()}};
...
window.addEventListener('beforeunload',e=>{if((M.editor&&(M.editor.saving||M.editor.processing||Object.values(M.editor.unfinishedLabels||{}).some(v=>v.trim())||Object.keys(M.editor.pending).length))||smallDirty()||M.small?.saving){e.preventDefault();e.returnValue=''}});
...
/* ADDRESSBOOK-EXTRAS-UI */
/* ADDRESSBOOK-EDITOR-UI */
/* ADDRESSBOOK-DEMO-CASES */
})();
