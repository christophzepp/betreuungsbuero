...
async function choose(c){if(!await flush())return;M.editRequest++;M.editor=null;M.utility=false;M.selected=identity(c);M.tab='data';M.root.classList.add('am-show-detail');window.phase5RenderAddressbookV154()}
...
function drawTab(c){
 ...
 if(M.tab==='sync'){drawSync(body,c);return}else if(M.tab==='ways'){drawWays(body,c,data);return}else if(M.tab==='addresses'){drawAddresses(body,c,data);return}else if(M.tab==='availability'){drawAvailability(body,c,data);return}else if(M.tab==='people'){
...
window.__abValidateImport=function(c){
 ...
 if(c.contactWays!=null)window.__abContactTools.ways(c.contactWays);for(const p of c.people||[])if(p.contactWays!=null)window.__abContactTools.ways(p.contactWays);dates(c);return c;
};
...
async function showTrash(){M.utility=true;
 ...
 for(const item of result.items){const s=section(item.label);s.append(E('p','am-sub',[dateLabel(item.deletedAt),item.actor,...(item.cases||[]).map(x=>x.label)].filter(Boolean).join(' · ')),B('Wiederherstellen',async()=>{
...
async function signaturePreview(proposal,c,text){
 ...
 pane.append(form,B('Als neuen Kontakt prüfen',async()=>{const patch=values(false);if(!c)Object.assign(patch,values(false));patch.contactWays=(proposal.contactWays||[]).filter(w=>patch[w.type]===w.value||w.preferred===false).map(w=>({...w,id:crypto.randomUUID()}));await edit(null,patch);await checkDuplicates(M.editor)},'primary'));
 if(c)pane.append(B('Ausgewählten Kontakt ergänzen',async()=>{const patch=values(false);if(!Object.keys(patch).length)throw Error('Bitte mindestens eine Angabe auswählen.');let bundle;if(online())bundle=await request('contact',{...ref(c),version:base.version,patch},'PATCH');else bundle=await savePatch(c,patch);await cacheBundle(bundle,ref(c));window.showImportedAddressbook();message('Ausgewählte Angaben übernommen')}));
...
function drawWays(body,c,d){
 const p=person(),profile=p||d;body.append(E('p','am-sub',(p?p.name+' · ':'')+'Benannte E-Mail-Adressen und Rufnummern. Der bevorzugte Weg je Art wird für die bestehenden Kontaktaktionen verwendet.'));
 for(const type of Object.keys(window.__abContactTools.TYPES))if(profile[type])info(body,'Standard · '+window.__abContactTools.TYPES[type],profile[type],type==='email'?undefined:'phone');
 for(const w of profile.contactWays||[]){const s=section(w.label);info(s,window.__abContactTools.TYPES[w.type],w.value,w.type==='email'?undefined:'phone');if(w.type==='email')s.append(B('E-Mail an diese Adresse',()=>mail(c,p,true,w.value)));s.append(B(w.preferred?'✓ Bevorzugt':'Als bevorzugt verwenden',()=>saveWays(c,p,(profile.contactWays||[]).map(x=>({...x,preferred:x.id===w.id||x.type!==w.type&&x.preferred})))),B('Kontaktweg bearbeiten',()=>editWay(c,p,w)),B('Kontaktweg entfernen',async()=>{if(confirm('Kontaktweg „'+w.label+'“ entfernen?'))await saveWays(c,p,profile.contactWays.filter(x=>x.id!==w.id))},'danger'));body.append(s)}body.append(B('+ Kontaktweg',()=>editWay(c,p,null),'primary'));
}
...
function editWay(c,p,w){const id=w?.id||crypto.randomUUID();let baseline=M.bundle;
 smallForm(w?'Kontaktweg bearbeiten':'Neuer Kontaktweg',[['type','Art',w?.type||'email',Object.entries(window.__abContactTools.TYPES)],['label','Bezeichnung',w?.label||''],['value','Adresse oder Rufnummer',w?.value||'']],async v=>{
