...
function mount(){
 ...
 const more=E('details','am-menu');more.append(E('summary','am-btn','Weitere Aktionen'));
...
async function showTrash(){M.utility=true;
 const root=M.root,pane=$('.am-detail');M.editor=null;M.small=null;M.load++;M.root.classList.add('am-show-detail');pane.classList.remove('am-editing');pane.replaceChildren(E('h2','','Papierkorb'),E('p','am-sub','Gelöschte Kontakte lassen sich mit ihren ursprünglichen Verknüpfungen wiederherstellen. Synchronisationen bleiben anschließend pausiert.'));
 const result=online()?await request('trash'):{items:state.ui.addressbookTrash||[]};if(M.root!==root)return;
...
async function signaturePreview(proposal,c,text){
 const pane=$('.am-detail');pane.replaceChildren(E('h2','','Erkannte Angaben prüfen'),E('p','am-sub','Die Erkennung macht Vorschläge. Angaben können korrigiert oder abgewählt werden. Bei einem vorhandenen Kontakt sind nur leere Felder vorausgewählt.'));
 const base=c?(online()?(await request('contact?'+new URLSearchParams(ref(c)))):{contact:c}):null,form=E('form','am-edit'),checks=new Map();
 ...
 if(c)pane.append(B('Ausgewählten Kontakt ergänzen',async()=>{const patch=values(false),ways=selectedWays(patch);if(ways.length){const before=base.contact.contactWays||[];patch.contactWays=[...before,...ways.filter(w=>!before.some(x=>x.type===w.type&&x.value===w.value)).map(w=>({...w,preferred:w.preferred&&!before.some(x=>x.type===w.type&&x.preferred)}))]}if(!Object.keys(patch).length)throw Error('Bitte mindestens eine Angabe auswählen.');let bundle;if(online())bundle=await request('contact',{...ref(c),version:base.version,patch},'PATCH');else bundle=await savePatch(c,patch);await cacheBundle(bundle,ref(c));window.showImportedAddressbook();message('Ausgewählte Angaben übernommen')}));
...
window.__abModern={extraFilter:(rows,v)=>X.filter(rows,v),mount,render,jumpToLetter,active:visible,hold:()=>!!(visible()&&(M.editor||M.small)),selected,flush,edit,savePatch};
