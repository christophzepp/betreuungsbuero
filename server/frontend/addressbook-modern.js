...
function info(parent,label,value,kind){if(!value)return;const wrap=E('div','am-info'),copy=B('Kopieren',()=>navigator.clipboard.writeText(String(value)),'quiet');copy.setAttribute('aria-label',label+' kopieren');const content=E('div');content.append(E('span','am-sub',label));
 if(kind==='address'&&window.__mapAddressLinkHTML){const d=E('div');d.innerHTML=window.__mapAddressLinkHTML(String(value));content.append(d)}else if(kind==='fax')content.append(faxLink(value));else if(kind==='phone'){const a=E('a','am-contact-link',value);a.href='tel:'+String(value).replace(/[^+\d]/g,'');a.onclick=()=>{if(selected())X.used(selected())};content.append(a)}else content.append(E('div','',value));wrap.append(content,copy);parent.append(wrap)}
function drawDetail(c){
 ...
 const tabs=E('nav','am-tabs');tabs.setAttribute('aria-label','Kontaktdetails');for(const [key,label] of [['data','Kontaktdaten'],['cases','Fälle & Standard'],['people','Ansprechpartner'],['addresses','Anschriften'],['ways','Kontaktwege'],['extras','Organisation & Extras'],['sync','Synchronisation'],['availability','Erreichbarkeit'],['history','Änderungen'],['communication','Kommunikation']]){if(c.__viewMergeId&&key!=='data')continue;const b=B(label,()=>{M.tab=key;drawTab(c);if(innerWidth<=680)requestAnimationFrame(()=>{const pane=$('.am-detail'),tabs=$('.am-tabs');if(pane&&tabs)pane.scrollTop+=tabs.getBoundingClientRect().top-pane.getBoundingClientRect().top})});b.dataset.tab=key;tabs.append(b)}pane.append(tabs,E('div','am-tab-body'));
 const more=E('details','am-more');more.append(E('summary','am-btn','Weitere Kontaktaktionen'));const other=E('div','am-detail-actions');
 other.append(B('Daten kopieren',()=>copyContact(c)));if(!c.__viewMergeId)other.append(B(isEnded(c)?'Aktivieren':'Beenden',()=>saveStatus(c,!isEnded(c))),B('Löschen',()=>removeContact(c),'danger'));
...
function drawTab(c){
 const body=$('.am-tab-body');if(!body||M.editor||M.small||M.utility)return;body.replaceChildren();for(const b of M.root.querySelectorAll('[data-tab]')){const active=b.dataset.tab===M.tab;b.setAttribute('aria-pressed',String(active));}
