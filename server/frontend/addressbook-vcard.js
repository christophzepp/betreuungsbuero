...
(function(root,factory){const codec=factory();if(typeof module==='object'&&module.exports)module.exports=codec;else root.__abVcardCodec=codec})(typeof window==='object'?window:globalThis,function(){
 ...
 function clean(source){
  const out=texts(source,scalar);dates(out);
  for(const [key,fields,max,required] of [['addresses',addressFields,30,'label'],['people',personFields,100,'name']])if(Object.hasOwn(source,key)){
   ...
   out[key]=source[key].map(v=>{const row=texts(v,fields,1000);if(!row.id||row.id.length>128||ids.has(row.id)||!row[required]?.trim())fail('Fehlende oder doppelte Kennung in '+key+'.');ids.add(row.id);dates(row);return row});
  }
 ...
 function stringify(source){
  ...
  if(c.email)lines.push('EMAIL;TYPE=INTERNET'+pref('email')+':'+esc(c.email));
  for(const [field,type] of [['phone','WORK,VOICE'],['mobile','CELL'],['fax','FAX']]){const v=c[field]||[c[field+'Area'],c[field+'Number']].filter(Boolean).join('/');if(v)lines.push('TEL;TYPE='+type+pref(field)+':'+esc(v))}
  ...
  for(const p of c.people||[]){const agent=['BEGIN:VCARD','VERSION:3.0','FN:'+esc(p.name),'N:'+esc(p.name)+';;;;'];if(c.institution||p.department)agent.push('ORG:'+esc(c.institution)+';'+esc(p.department));if(p.role)agent.push('TITLE:'+esc(p.role));if(p.email)agent.push('EMAIL;TYPE=INTERNET:'+esc(p.email));if(p.phone)agent.push('TEL;TYPE=WORK,VOICE:'+esc(p.phone));if(p.fax)agent.push('TEL;TYPE=FAX:'+esc(p.fax));agent.push('END:VCARD');lines.push('AGENT:'+esc(agent.join('\n')))}
 ...
 function parse(text,depth=0){
  ...
  return cards.map(props=>{
   ...
   const c={},addresses=[],people=[],rank={};let fn='',preferredChannel='';
   for(const p of props){const value=unesc(p.value),params=p.params.toUpperCase().replace(/"/g,''),preferred=/(?:^|[=;,])PREF(?:[;,]|$)|(?:^|;)PREF=1(?:;|$)/.test(params),put=(field,v)=>{if(c[field]==null||preferred&&!rank[field]){c[field]=v;rank[field]=preferred}};
    ...
    else if(p.name==='AGENT'&&!/VALUE\s*=\s*"?(URI|TEXT)/i.test(params)){const agent=parse(value,depth+1)[0];if(agent){const org=value.split(/\r?\n/).map(property).find(x=>x?.name==='ORG');people.push({id:'person-'+people.length,name:[agent.title,agent.firstName,agent.lastName].filter(Boolean).join(' ')||agent.institution||'Ansprechpartner',department:org?unesc(split(org.value)[1]||''):'',role:agent.role||'',email:agent.email||'',phone:agent.phone||'',fax:agent.fax||''})}}
   }
   if(!c.institution&&!c.firstName&&!c.lastName)c.institution=fn;
   if(addresses.length){c.addresses=addresses.map(({preferred,...a})=>a);const chosen=addresses.find(a=>a.preferred)||addresses[0];c.preferredAddressId=chosen.id;for(const key of ['street','city','postal','postbox'])c[key]=chosen[key]}
   if(people.length)c.people=people;if(preferredChannel)c.preferredChannel=preferredChannel;
   return clean(c);
