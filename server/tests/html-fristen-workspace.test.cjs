...
test('Bestätigtes Speichern meldet Schreibschutz, falschen Fall und Serverfehler',async()=>{
 const start=html.indexOf('  async function flushStammdatenSync('),end=html.indexOf('  function diffReportFields(',start),ctx={window:{__appMode:'online'},Blob,clearTimeout(){},stammdatenEpoch:0,stammdatenFlight:null,stammdatenTimer:null,stammdatenPendingSince:0,stammdatenRetryTimer:null,stammdatenRetryDelay:2000,stammdatenSaveStatus:{state:'idle'},caseSaveStatus(){},retryStammdaten(){},applyPatchesToObject:(o,p)=>p.forEach(x=>o[x.path]=x.value),currentCaseId:'case-a',lastSyncedCaseData:{},combinedStammdatenView:()=>({fristen:[{id:'a'}]}),diffTopLevel:o=>o.fristen?[]:[{path:'fristen',value:[{id:'a'}]}],STAMMDATEN_EXCLUDE:[],rememberSent:()=>{},clone:structuredClone,warnPermissionDeniedOnce:()=>{},fetch:async()=>({ok:false,status:503})};vm.createContext(ctx);vm.runInContext(html.slice(start,end),ctx);
 await assert.rejects(()=>ctx.flushStammdatenSync({strict:true,caseId:'case-b'}),/Zielfall/);ctx.window.__mwReadOnly=true;await assert.rejects(()=>ctx.flushStammdatenSync({strict:true,caseId:'case-a'}),/schreibgeschützt/);ctx.window.__mwReadOnly=false;await assert.rejects(()=>ctx.flushStammdatenSync({strict:true,caseId:'case-a'}),/503/);assert.deepEqual(ctx.lastSyncedCaseData,{});await ctx.flushStammdatenSync();ctx.fetch=async()=>({ok:true,json:async()=>({ok:true})});await ctx.flushStammdatenSync({strict:true,caseId:'case-a'});assert.equal(ctx.lastSyncedCaseData.fristen[0].id,'a');
});
...
test('Fehlgeschlagenes Nachladen erhält die bisherigen Fristenvorschläge',async()=>{
 const start=script.indexOf('async function loadCases('),end=script.indexOf('window.openFristenModal=',start),ctx={window:{},frLoadRetainedDeadlines:async()=>{},halfYearStart:()=> '2026-07-01',encodeURIComponent,frItems:[{caseId:'bisher'}],frCasesLoaded:true,fetch:async()=>({ok:false,status:503})};vm.createContext(ctx);vm.runInContext('async '+source('fristCases','__caseDeadlines')+script.slice(start,end),ctx);await assert.rejects(()=>ctx.loadCases(true),/503/);assert.equal(ctx.frItems[0].caseId,'bisher');ctx.fetch=async()=>({ok:true,json:async()=>({items:[{caseId:'neu'}]})});await ctx.loadCases(true);assert.equal(ctx.frItems[0].caseId,'neu');
});
