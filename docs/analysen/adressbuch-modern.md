...
Weitere geladene Fälle erscheinen bei ausdrücklich gespeicherter zentraler Verknüpfung. Gleiche Namen oder kopierte Kontaktkennungen allein begründen keine zusätzliche Zuordnung. Lokale Bürokontakte ohne Verknüpfung zeigen einen passenden Leerzustand. Rolle, Aktenzeichen, Vorgangs- und Kundennummer sowie bereits hinterlegte Standards bleiben pro Fall getrennt sichtbar. Neue zentrale Zuordnungen und das Festlegen von Standards werden weiterhin online verwaltet. Die lokale Anzeige sendet keine Adressbuch-Anfragen an den Server.

`QA_LOCAL_CASES=1 node server/scripts/qa-addressbook-modern.cjs` prüft Demo-Kontakte ohne ID, aktuelle gegenüber veralteten Falldaten, Referenzänderungen, ausdrückliche zentrale Verknüpfungen, Namens-/ID-Kollisionen, nicht zugeordnete Bürokontakte und unveränderte Online-Verwaltungsaktionen auf Desktop und Mobil.
